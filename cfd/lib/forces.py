...
# Сколько точек обязано быть в окне усреднения, чтобы среднее было средним.
# Силы пишутся каждый шаг, поэтому у любого здорового окна их сотни; пять —
# это порог «хоть что-то», ниже которого разброс и дрейф считать бессмысленно.
MIN_WINDOW = 5
...
def window(t, start=None, frac=0.5):
    ...
    # ОКНО НЕ СХЛОПЫВАЕТСЯ МОЛЧА. Прежде тут стоял откат на две последние
    # точки, и он однажды соврал так, что дороже не придумаешь: тонкая
    # эталонная сетка остановилась по residualControl на 1647-м шаге при окне,
    # заданном с 5200-го. Окно выродилось в ДВЕ соседние итерации, разброс
    # вышел 1e-4, дрейф — ровно ноль, и сводка объявила прекрасно сошедшееся
    # число, снятое с середины переходного процесса: Cl 0.917 там, где сила
    # ещё росла и шла к 1.08 (§13.25).
    #
    # Правильный ответ здесь — отказ. Среднее по двум точкам не среднее, а
    # мгновенное значение с видом среднего, и это худший из возможных
    # результатов: он выглядит достовернее, чем честное «не сошлось».
    if len(idx) < MIN_WINDOW:
        raise ValueError(
            "окно усреднения с %g не набрало точек: расчёт кончился на %g "
            "(нашлось %d, нужно хотя бы %d). Либо расчёт остановлен раньше "
            "окна, либо окно задано за его пределами; среднее по двум точкам "
            "тут было бы обманом." % (start, t[-1], len(idx), MIN_WINDOW))
...
def stats(t, x, start=None, frac=0.5):
    ...
    return {
        ...
        # Знаменатель поднят до порога: при нулевом разбросе отношение
        # честно бесконечно, но json.dump пишет литерал Infinity, который не
        # читает ничто, кроме самого питона. Порог здесь — минимальный
        # различимый разброс, так что смысл «тренд при мёртвом разбросе
        # огромен» сохраняется, а сводка остаётся валидным JSON.
        "drift": 0.0 if trend <= floor else trend / max(std, floor),
        # Дрейф относительно САМОЙ ВЕЛИЧИНЫ, а не её разброса.
        #
        # Обе меры нужны, и ни одна не заменяет другую. Отношение к разбросу
        # отвечает на вопрос «тренд это или дрожь»; отношение к среднему — на
        # вопрос «а есть ли до этого дело». У сошедшегося киля разброс силы
        # 0.003 Н при силе 439 Н, и микротренд в полсотой ньютона давал
        # «дрейф 1.9» — тревогу на ровном месте. Дрейфующей сила считается
        # только когда обе меры велики разом.
        "trend_frac": trend / max(abs(mean), 1e-12),
...
_LAYERS = re.compile(r"Added (\d+) out of (\d+) cells \(([-+0-9.eE]+)%\)")


def read_layers(log_path):
    """Сколько ячеек пограничного слоя сеточник добавил на самом деле.

    Число это молчаливое и потому опасное. `snappyHexMesh` не считает неудачу с
    слоями ошибкой: он пишет «Added 0 out of 582864 cells (0%)» и идёт дальше.
    Сетка получается, решатель считает, силы выходят — и только трение при этом
    посчитано на голых ячейках без разрешения пристеночной области. На тонком
    сечении паруса именно так и вышло.

    Берётся ПОСЛЕДНЕЕ значение: строк несколько (снятие, притягивание, слои), и
    смысл имеет итог.
    """
    last = None
    with open(log_path, encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            if m:
                last = {"added": int(m.group(1)), "of": int(m.group(2)),
