...
def sail_section(camber, draft=0.5, chord=1.0, thickness=0.015, n=200,
    ...
    Профиль толщины — ИЗОГНУТАЯ ПЛАСТИНА, а не веретено NACA: постоянная
    толщина вдоль всей хорды, полукруглый нос радиусом в полтолщины, прямой
    обрез хвоста. Первая версия одевала среднюю линию распределением толщины
    NACA, и это дало остриё с носовым радиусом 1.1019·t² — при листе в полтора
    процента хорды ровно один миллиметр. Сетка такое остриё не разрешает ни на
    каком разумном уровне: у всех четырёх сечений checkMesh находил пару
    десятков рваных граней (skewness до 9.4 при пороге 4), и убирание
    пристеночных слоёв их не трогало — грани сидели на кромках. Ткань к тому же
    микроострия не образует: мягкая передняя шкаторина принимает радиус порядка
    своей толщины, задняя подшита. Пластина постоянной толщины — та же самая
    ...
    dyc = np.where(
        ...
        2 * camber / (1 - p) ** 2 * (p - x))
    r = 0.5 * thickness
    yt = np.where(x < r, np.sqrt(np.maximum(0.0, r * r - (x - r) ** 2)), r)
    # У самого носа полутолщина нулевая, и обе поверхности сходятся в одну
    # точку — там контур и замыкается.
    th = np.arctan(dyc)
...
def sign_probe(size=1.0):
    ...
    Координаты вершин НЕКРУГЛЫЕ, и это выстрадано: первая версия ставила клин
    на 1.0 / −0.5 / 0.35, и при базовой ячейке в полметра его грани легли ровно
    в плоскости blockMesh. Пересечения рёбер сетки с такой поверхностью
    вырождаются — snappy нашёл ДВА пересечения из четверти миллиона рёбер, не
    вырезал тело вовсе и молча отдал домен с одним патчем farfield: сила по
    всем осям вышла нулём. Смещение на нецелые миллиметры не значит для пробы
    знаков ничего, а совпадение с узлами сетки исключает при любом разумном
    шаге.
    """
    v = np.array([
        [1.0137, 0.0231, 0.0173], [-0.4863, 0.3731, -0.1827],
        [-0.4863, -0.1269, -0.1827], [-0.4863, 0.1731, 0.4673],
        [-0.6863, 0.0231, 0.0173],
    ]) * size
...
def volume_below(tris, z0=0.0):
    """Объём замкнутого тела ниже плоскости z = z0. Считается ТОЧНО.

    По теореме о дивергенции для поля (0, 0, z − z0) объём равен интегралу
    (z − z0)·n_z по замкнутой поверхности. Крышка на самой плоскости вклада не
    даёт — там z − z0 = 0, — поэтому достаточно обойти обрезанные части
    треугольников, и никакой выборки не нужно: у плоского многоугольника
    среднее линейной функции равно её значению в центре тяжести.

    Нужно это ради свидетеля на гидростатику: архимедова сила погружённого
    тела известна заранее с точностью до объёма его геометрии, и это
    единственная величина во всём контуре со свободной поверхностью, которую
    можно проверить, ничего не считая.
    """
    total = 0.0
    for t in np.asarray(tris, dtype=np.float64):
        poly = _clip_below(t, z0)
        if len(poly) < 3:
            continue
        # Векторная площадь многоугольника и его центр тяжести.
        av = np.zeros(3)
        cz, aw = 0.0, 0.0
        for i in range(1, len(poly) - 1):
            tri = (poly[0], poly[i], poly[i + 1])
            n = np.cross(tri[1] - tri[0], tri[2] - tri[0]) * 0.5
            av += n
            a = np.linalg.norm(n)
            cz += a * (tri[0][2] + tri[1][2] + tri[2][2]) / 3.0
            aw += a
        if aw <= 0.0:
            continue
        total += av[2] * (cz / aw - z0)
...
def _clip_below(tri, z0):
    """Часть треугольника ниже z0 как многоугольник (0, 3 или 4 вершины)."""
    out = []
    for i in range(3):
        a, b = tri[i], tri[(i + 1) % 3]
        if a[2] <= z0:
            out.append(a)
        if (a[2] - z0) * (b[2] - z0) < 0.0:
            t = (z0 - a[2]) / (b[2] - a[2])
            out.append(a + (b - a) * t)
    return out
