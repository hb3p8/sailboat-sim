...
def context(m, geometry_dir=None):
    ...
    # ЯВНОЕ задание величин на дальней границе — оно перекрывает назначенные
    # выше и нужно там, где их кто-то нормирует.
    #
    # Умолчание выше даёт отношение вихревой вязкости к молекулярной около
    # четырёх тысяч. Для внешней задачи это чудовищно много: TMR для
    # стандартной SST требует, чтобы оно лежало между 1e-5 и 1e-2, — то есть
    # мы выше верхней границы в сотни тысяч раз. На домене в двадцать хорд это
    # сходило с рук, потому что поток доносит турбулентность до тела почти
    # нетронутой; на пятистах хордах эталонной сетки решение вышло сорванным.
    #
    # Значения не пересчитываются по формуле, а берутся из манифеста: у
    # источника эталона они заданы диапазоном, и выбор внутри него — часть
    # постановки, которую надо видеть в манифесте, а не выводить из кода.
    t = m.get("fluid", {})
    if "k_farfield" in t or "omega_farfield" in t:
        if not ("k_farfield" in t and "omega_farfield" in t):
            raise TemplateError("fluid: k_farfield и omega_farfield задаются "
                                "только вместе — иначе отношение вязкостей "
                                "выходит случайным")
        k, omega = float(t["k_farfield"]), float(t["omega_farfield"])
...
def _mesh_context(m):
    ...
    return {
        ...
        "body_box_min": " ".join("%g" % v for v in mesh.get(
        ...
        # в шаблон список означал бы, что вторая сетка молча поедет по чужому
        # правилу — ровно тот класс ошибки, что уже стоил разворота профиля
        # задом наперёд.
        "grid_transform_block": "\n".join(
            "runApplication transformPoints %s" % t
