...
QUEUE = [
    ...
    # Два опыта по §13.17: откуда у NACA избыток сопротивления давления.
    # Каждый меняет РОВНО ОДИН множитель при прочих равных, поэтому разность
    # ответов внутри группы и есть искомая чувствительность. Околотельная
    # сетка у обоих опытов нарочно грубее рабочей тройки: сравниваются случаи
    # между собой, а не с эталоном TMR.
    ("naca-dom-020", GEOM_MAIN),
    ("naca-dom-040", GEOM_MAIN),
    ("naca-dom-080", GEOM_MAIN),
    ("naca-yplus-40", GEOM_MAIN),
    ("naca-yplus-01", GEOM_MAIN),
    # Тройка на ЭТАЛОННЫХ сетках NASA TMR (§13.19): та же задача, чужая
    # сетка, разрешённый пограничный слой и турбулентность на границе,
    # заданная по руководству источника. Разделяет две ошибки, которые у нас
    # до сих пор слиты: постановка сетки и решатель с моделью.
    ("naca-tmr-coarse", GEOM_MAIN),
    ("naca-tmr-medium", GEOM_MAIN),
    ("naca-tmr-fine", GEOM_MAIN),
    # Сечение генакера на СВОЕЙ сетке по телу (§13.27): тот же угол и то же
    # пузо, что у зонда, но кольца по обшивке вместо декартова выреза и
    # дальняя граница 500 хорд вместо 5.1.
    ("gen-og-a35", GEOM_SAIL),
    # Развёртка по дальней границе на ДЕКАРТОВОЙ сетке: 60 хорд против 20 у
    # рабочего случая, всё прочее неизменно (§13.30).
    ("gen-dom-60", GEOM_SAIL),
]
