...
QUEUE = [
    # Решающий опыт на симметрию — первым. Поляра киля при обратной
    # экстраполяции даёт на нулевом угле полтораста ньютонов боковой силы, а
    # обязана давать ноль; такое же смещение нашлось у корпуса. Пока не
    # известно, дефект это постановки или нелинейность поляры, читать остальные
    # точки нельзя, и тратить на них часы тем более.
    ("keel-u200-a00-medium", GEOM_KEEL),
    # цепочка и метод: плоский профиль против опубликованного набора
    ("naca0012-a10-coarse", GEOM_MAIN),
    # знаки на каноническом теле (§5, этап 0.6). Пара дешёвая — грубая сетка на
    # клине, — а закрывает то, что иначе всплывает через неделю необъяснимым
    # расхождением: перестановку компонент силы где-нибудь в постобработке.
