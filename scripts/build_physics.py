...
# своими пределами врёт. Её место здесь, отдельной строкой, чтобы было видно,
# что это калибровка, а не расчёт.
#
# Подбирается по наблюдению владельца: галфвинд в 11.7 узлах ветра у рига,
# экипаж на борту, парус настроен — около 8.5 узла. Одним множителем на всю
# кривую; форма не трогалась.
#
# Пересобрана 02.08.2026, множитель 0.72. Прежняя кривая подгонялась под риг,
# который потом дважды оказался неправильным: сперва в нём было 25 м² вместо
# снятых с чертежа 22.85, потом выяснилось, что индуктивное сопротивление
# считалось по полному местному скосу и выходило заниженным. Оба раза лодка
# теряла ход, и оба раза кривую сознательно не трогали, чтобы не смешивать
# исправление ошибки с подгонкой. Теперь риг починен, и подгонку можно
# доделать.
#
# Что получилось в долях веса вместе с трением (R/W): 0.048 на 5.8 узла, 0.093
# на 7.8, 0.150 на 11.7, 0.196 на 15.6. Для лёгкого глиссирующего корпуса это
# нормальный коридор — качество на глиссировании выходит около пяти. Прежняя
# кривая давала на 7.8 узла 0.126, то есть корпус был тяжеловат.
#
# Форма кривой по-прежнему нарисована от руки, и это следующее, что стоит
# заменить: волновое сопротивление считается офлайн по обводам (тонкое судно,
# интеграл Мичелла), и тогда подгоночным останется один множитель.
RESIDUARY = [
