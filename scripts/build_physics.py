...
def _deadrise(sec_x, sec_poly, lwl_aft, lwl_fwd):
    ...
    Мерится наклон хорды днища от киля до точки обвода на высоте В ПОЛОВИНУ
    ОСАДКИ ЭТОГО ШПАНГОУТА (но не выше 0.10 м). Высота обязана быть сомасштабна
    погружению, и первая версия на этом обожглась: она мерила на фиксированных
    0.20 м, а кормовые шпангоуты сидят на 18…150 мм — точка на 0.20 м лежала уже
    на завале борта, и выходило 12.65° там, где днище идёт под 3.7…4.1°. Ловится
    это сверкой с параметром обводов: у подгонки killеватость кормы beta около
    четырёх градусов, и замер обязан с ней сходиться — оба смотрят на одну
    поверхность.
    """
    out = []
    ...
    for x, poly in zip(sec_x, sec_poly):
        ...
        rise_m = min(0.10, 0.5 * max(1e-3, -zmin))
        zc = zmin + rise_m
        edge = None
        ...
        dy = abs(edge - ykeel)
        if dy > 1e-6:
            out.append(math.degrees(math.atan2(rise_m, dy)))
...
# Вынос фала генакера ВПЕРЁД от мачты, м.
#
# Не украшение и не подгонка, а устранение особенности. Фал генакера и фаловый
# угол грота иначе оказываются в одной точке: топ мачты (3.3814, 9.417) и
# фаловый угол грота (3.3848, 9.3423) — семь сантиметров. Для решётки вихрей две
# несущие поверхности, сходящиеся в точку, это вырождение: контрольная точка
# одной садится на присоединённый вихрь другой. Замер прямой: ближайшая пара
# панелей стояла в шести сантиметрах при размере панели 1.38 м.
#
# У настоящей лодки этого нет: фал генакера выходит из мачты через кран или
# ролик, вынесенный вперёд, — иначе парус тёрся бы о грот на каждом обороте.
# Тридцать сантиметров — обычный вынос такого крана.
GENNAKER_HEAD_FWD = 0.3

# Ширина паруса у фала, м. У ГРОТА ОНА 0.136, У СТАКСЕЛЯ 0.028, а у генакера
# была ровно ноль — и это оказалось не мелочью кроя, а вырождением расчёта.
#
# Полоска у фала при нулевой ширине получает хорду по нижнему пределу — пять
# сантиметров, — а панелей вдоль хорды три, то есть панель длиной в полтора
# сантиметра при радиусе вихревого ядра в пять. Контрольная точка садится внутрь
# ядра собственного присоединённого вихря, матрица влияния вырождается, и
# циркуляция улетает. Замер прямой: генакер В ОДИНОЧКУ, без грота, разгонял узел
# пелены до 5091 м/с при кажущемся ветре 4 м/с, и всегда на верхних нитях.
#
# У настоящего паруса точки в фаловом углу нет: там либо фаловая дощечка, либо
# короткая шкаторина. Четверть метра для паруса такой площади — обычная.
GENNAKER_HEAD_W = 0.25
...
# Отношения обводов генакера. Чертежа паруса не существует, и взяты они из
# обмерных правил, а не из головы.
#
# СРЕДНЯЯ ШИРИНА К НИЖНЕЙ. Порог 0.75 — не «типовое значение», а определение:
# по правилам ORC парус считается спинакером, если SHW ≥ 0.75·SFL, иначе это
# генуя. То есть 0.75 — нижняя граница класса, и кроить по ней значит получить
# самый плоский парус, какой ещё называется спинакером. Настоящий несимметричный
# «широкоплеч»: у ходовых 0.85…0.90, у бегущих средняя ширина превышает нижнюю.
# Владелец носит его в основном на бакштаге и иногда близко к фордевинду — 0.85.
#
# ЗАДНЯЯ ШКАТОРИНА КОРОЧЕ ПЕРЕДНЕЙ: парус скручен вперёд. Это БОЛЬШЕ НЕ ВХОД:
# длина задней теперь следует из обвода — из положения фала, шкотового угла и
# своего серпа, — а не назначается долей от передней. Отношение, которое при
# этом выходит, около 0.88, то есть прежняя оценка 0.90 была недалека; но
# назначать его было нечем, а теперь и незачем.
GENNAKER_GIRTH_K = 0.85

# Серпы шкаторин.
#
# Лишняя ширина — та, которой обмерные 0.85·SFL превышают половину нижней
# шкаторины, — набирается ПЕРЕДНЕЙ шкаториной, а не задней. Почему именно так и
# чем это померено — в `_gennaker`.
#
# Серп передней здесь НЕ ЗАДАН: он подбирается под обмерную среднюю ширину, и
# выходит около 0.14 длины передней шкаторины. План §Б1 оценивал 0.10…0.12, не
# делая этой арифметики; расхождение записано, а не сглажено.
#
# Серп задней — ОЦЕНКА, и мелкая: у гоночных несимметричных задняя шкаторина
# почти прямая, у иных слегка вогнутая. Три процента нижней шкаторины — верх
# того, что бывает у ходового паруса.
LEECH_ROUND_K = 0.03

# Где стоит плечо — самое широкое место паруса, в долях высоты. У летящего
# несимметричного оно заметно выше середины (§Б0.2 плана генакера); 0.60…0.65 —
# то, что видно на фотографиях полётных форм.
LUFF_ROUND_AT = 0.62

# Точек в ломаной шкаторины. Девятью серп читался ломаным у плеча, семнадцать
# дают гладкую дугу и стоят те же доли миллисекунды на сборке пакета.
GENNAKER_PTS = 17
...
def _gennaker(sp, forestay_stem_mm, mast_top_mm):
    ...
    head = (mast_top_mm[0] + GENNAKER_HEAD_FWD * 1000.0, mast_top_mm[1])
    tk, hd = (tack[0] / 1000.0, tack[1] / 1000.0), (head[0] / 1000.0, head[1] / 1000.0)
    span = hd[1] - tk[1]
    straight = math.hypot(hd[0] - tk[0], span)      # прямая галс—фал
    head_aft_x = hd[0] - GENNAKER_HEAD_W
    # Нормаль к прямой галс—фал, наружу вперёд: по ней откладывается серп
    # передней шкаторины.
    nx, nz = span / straight, -(hd[0] - tk[0]) / straight

    # ОБВОД: ЛИШНЯЯ ШИРИНА ОТ ПЕРЕДНЕЙ ШКАТОРИНЫ, А НЕ ОТ ЗАДНЕЙ.
    #
    # У треугольника с этим галсом средняя ширина выходит ровно половиной нижней
    # шкаторины (следствие того, что галс вынесен вперёд), а по обмеру нужно
    # 0.85. Недостающее — серпы, и весь вопрос в том, какой шкаториной они
    # набраны. Были набраны задней целиком, и это неверно вдвойне.
    #
    # Во-первых, так паруса не кроят: у несимметричного задняя шкаторина почти
    # прямая или слегка вогнутая, а ширина набирается ПЕРЕДНЕЙ (luff round) и
    # округлой нижней. Во-вторых — и это уже замер, — серп в задней даёт ломаную
    # площадью 24.4 м² при обмерных 27.0: обвод не сходился сам с собой на
    # десять процентов, и модель это гасила приведением суммы полосок к площади
    # из пакета. С серпом передней невязка падает до четырёх процентов.
    #
    # Поймано тканью (docs/gennaker-sota-plan.md, §Б2): полотну на прежнем обводе
    # нет летящей формы — задняя шкаторина выходила 9.21 м при заявленных 8.39, и
    # весь этот метр уходил в складки.
    #
    # ЧТО ЗДЕСЬ ВАЖНО НЕ ПЕРЕПУТАТЬ. Серп передней шкаторины — это ВЫКРОЙКА, а
    # обвод в пакете — ПРЕДСТАВЛЕНИЕ ЛЕТЯЩЕГО ПАРУСА для полосок. В полёте
    # передняя шкаторина лежит на прямой галс—фал по X, а лишняя её длина уходит
    # вбок провисом; вперёд она не выдувается. Поэтому в пакет передняя идёт
    # ПРЯМОЙ, как и была, а серп меняет то, что и должен менять, — РАСПРЕДЕЛЕНИЕ
    # ШИРИНЫ ПО ВЫСОТЕ. Плечо уезжает с половины высоты на 0.62, парус становится
    # полнее вверху и уже внизу. Сама же величина серпа отдаётся наружу
    # (`luff_round_m`): из неё ткань берёт запас длины передней шкаторины, а
    # провис перестаёт быть назначенной постоянной (§Б0.2).
    #
    # ЧЕГО ЗДЕСЬ НЕТ: серпа нижней шкаторины, который план §Б1 тоже называет.
    # Обвод задан двумя функциями x(z), и выгнутая вниз нижняя в такой записи
    # непредставима: полоски режутся от zLo = max(галс, шкотовый), и всё, что
    # ниже, отрезается. Заводить ради неё третью кривую, которой никто не читает,
    # незачем.
    round_pow = math.log(0.5) / math.log(LUFF_ROUND_AT)

    def luff_round(f, amp):
        """Серп передней шкаторины на доле высоты f, поперёк прямой галс—фал."""
        return amp * math.sin(math.pi * f ** round_pow)

    def width(z, sfl, amp):
        """Ширина паруса на высоте z: треугольник плюс оба серпа."""
        f = (z - tk[1]) / span
        tri = (tk[0] + (hd[0] - tk[0]) * f) - (clew_x(sfl) +
              (head_aft_x - clew_x(sfl)) * f)
        return tri + luff_round(f, amp) * nx + LEECH_ROUND_K * sfl * math.sin(math.pi * f)

    def clew_x(sfl):
        return tk[0] - sfl

    def amp_for(sfl):
        """Серп передней — под обмерную среднюю ширину. Деление пополам."""
        lo, hi = 0.0, 6.0
        for _ in range(80):
            mid = (lo + hi) / 2.0
            if width(tk[1] + span / 2.0, sfl, mid) < GENNAKER_GIRTH_K * sfl:
                lo = mid
            else:
                hi = mid
        return (lo + hi) / 2.0
    ...
    def shape(sfl):
        """Ломаные обвода, длины шкаторин и обмерная площадь при данной нижней."""
        amp = amp_for(sfl)
        leech = []
        for k in range(GENNAKER_PTS):
            f = k / (GENNAKER_PTS - 1.0)
            z = tk[1] + span * f
            leech.append([tk[0] + (hd[0] - tk[0]) * f - width(z, sfl, amp), z])
        leech.reverse()                                  # сверху вниз, как было
        # Передняя ПО ВЫКРОЙКЕ — с серпом: это её настоящая длина, и её меряет
        # обмерщик. В пакет пойдёт прямая.
        cut = []
        for k in range(GENNAKER_PTS):
            f = k / (GENNAKER_PTS - 1.0)
            d = luff_round(f, amp)
            cut.append([tk[0] + (hd[0] - tk[0]) * f + d * nx, tk[1] + span * f + d * nz])
        slu = sum(math.hypot(b[0] - a[0], b[1] - a[1]) for a, b in zip(cut, cut[1:]))
        sle = sum(math.hypot(b[0] - a[0], b[1] - a[1]) for a, b in zip(leech, leech[1:]))
        shw = GENNAKER_GIRTH_K * sfl
        area = (slu + sle) / 2.0 * (sfl + 4.0 * shw) / 6.0
        return area, amp, slu, sle, shw, leech

    # Нижняя шкаторина — единственная неизвестная, когда площадь задана. Прежде
    # она находилась в одну строку, потому что SLE назначалась долей от SLU;
    # теперь обе длины СЛЕДУЮТ из обвода, и решать приходится делением пополам.
    # По площади оно монотонно, восьмидесяти шагов хватает с запасом.
    lo, hi = 1.0, 8.0
    for _ in range(80):
        mid = (lo + hi) / 2.0
        if shape(mid)[0] < SPINNAKER_M2:
            lo = mid
        else:
            hi = mid
    sfl = (lo + hi) / 2.0
    area, amp, slu, sle, shw, leech_f = shape(sfl)
    assert abs(area - SPINNAKER_M2) < 1e-6, area
    clew = (clew_x(sfl), tk[1])
    luff_pts = [[round(tk[0], 4), round(tk[1], 4)], [round(hd[0], 4), round(hd[1], 4)]]
    leech_pts = [[round(x, 4), round(z, 4)] for x, z in leech_f]
    # Обух шкота — на транцевом углу, у самого борта. Оттуда генакер и работают:
    # шкот идёт снаружи всего, мимо вант, и чем дальше в корму обух, тем шире
    # можно вынести парус.
    #
    # Берётся из обводов, а не назначается: кормовое сечение, его полуширота и
    # верх борта. Выходит (0.30, 0.965, 0.587) — угол транца.
    lead = [0.3, 0.965, 0.587]

    # Длина шкота от обуха до шкотового угла — ЕЮ парус и настраивается.
    #
    # Шкотовый угол ходит по дуге ВОКРУГ ОСИ ГАЛС—ФАЛ: выбрал шкот — угол пришёл
    # к диаметральной, потравил — ушёл наружу и вверх. Значит настройка у
    # генакера не угол, а ДЛИНА, и пределы её задаёт геометрия, а не вкус.
    #
    # Вокруг ОСИ, а не вокруг галса, и это не тонкость записи. Обе шкаторины,
    # нижняя и задняя, концами закреплены: нижняя на галсе, задняя на фале.
    # Значит |галс—шкот| и |фал—шкот| при любой постановке одни и те же, а
    # множество точек с двумя заданными расстояниями до двух точек — это
    # окружность вокруг прямой, их соединяющей. Всякая другая дуга рвёт одну из
    # шкаторин.
    #
    # Так здесь и было НАПИСАНО с самого начала, а посчитано другое:
    # горизонтальная окружность вокруг галса, на высоте галса. Она держит нижнюю
    # шкаторину и рвёт заднюю — |фал—шкот| растёт с 8.66 до 10.68 м при
    # заявленной SLE 8.39, то есть на рабочем шкоте на 9 %, на отданном на 27 %.
    # Поймано тканью (docs/gennaker-sota-plan.md, §Б2): на таком обводе полотну
    # просто нет летящей формы.
    #
    # Отсюда же само собой берётся то, чего модели не хватало и что назначать
    # было нечем: на правильной дуге шкотовый угол ПОДНИМАЕТСЯ над галсом — до
    # 1.94 м на отданном шкоте, — а не ползает по палубе (§Б0.4). Ни одной новой
    # постоянной для этого не понадобилось.
    # Орт оси галс—фал — по ПРЯМОЙ между ними, а не по длине шкаторины: та
    # теперь длиннее прямой на серп.
    ux, uz = (hd[0] - tk[0]) / straight, (hd[1] - tk[1]) / straight
    vx, vz = clew[0] - tk[0], clew[1] - tk[1]
    t_ax = vx * ux + vz * uz
    px, pz = vx - t_ax * ux, vz - t_ax * uz
    r_arc = math.hypot(px, pz)
    e1 = (px / r_arc, pz / r_arc)
    c_arc = (tk[0] + t_ax * ux, tk[1] + t_ax * uz)

    def clew_at(theta):
        """Шкотовый угол на дуге: (x, y, z). При theta = 0 он в ДП, у обвода."""
        return (c_arc[0] + r_arc * e1[0] * math.cos(theta),
                r_arc * math.sin(theta),
                c_arc[1] + r_arc * e1[1] * math.cos(theta))

    def sheet_len(theta):
        c = clew_at(theta)
        return math.hypot(c[0] - lead[0], c[1] - lead[1], c[2] - lead[2])

    # Наименьшая длина шкота — не при theta = 0, и это следствие дуги, а не
    # оговорка. Обух стоит на транцевом углу СБОКУ, поэтому ближайшая к нему
    # точка дуги чуть вынесена наружу; выбрать шкот короче нельзя ничем.
    theta_max = math.radians(110)
    grid = [i * theta_max / 2000 for i in range(2001)]
    theta_min = min(grid, key=sheet_len)

    return {
        "sheet_lead_m": lead,
        # Дуга шкотового угла — наружу целиком: симулятор обращает её каждый шаг,
        # и считать её там во второй раз значило бы завести второе описание
        # одной вещи.
        "clew_arc_c": [round(c_arc[0], 5), round(c_arc[1], 5)],
        "clew_arc_e1": [round(e1[0], 6), round(e1[1], 6)],
        "clew_arc_r": round(r_arc, 5),
        # Короче не бывает: ближайшая к обуху точка дуги.
        "sheet_min_m": round(sheet_len(theta_min), 3),
        # Дальше ста десяти градусов по дуге парус уже не несёт, а полощет: за
        # этим пределом травить нечего.
        "sheet_max_m": round(sheet_len(theta_max), 3),
        ...
        "head_aft": [round(hd[0] - GENNAKER_HEAD_W, 4), round(hd[1], 4)],
        "head_width_m": GENNAKER_HEAD_W,
        "clew": [round(clew[0], 4), round(clew[1], 4)],
        # Серп ПЕРЕДНЕЙ шкаторины: та самая ткань, которая в полёте уходит под
        # ветер. Отсюда же будет выводиться провис, когда он перестанет быть
        # постоянной (план §Б0.2).
        "luff_round_m": round(amp, 4),
        "luff_round_at": LUFF_ROUND_AT,
        "leech_round_m": round(LEECH_ROUND_K * sfl, 4),
        "luff": luff_pts,
        "leech": leech_pts,
        "sprit_m": SPRIT_M,
        "luff_m": round(slu, 4),
