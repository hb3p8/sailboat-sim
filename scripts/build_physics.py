...
# Вязкость воздуха нужна для числа Рейнольдса полоски паруса — по нему берётся
# поляра сечения (docs/flow-plan.md, Часть II). Значение при 15 °C: при
# парусных хордах в метр-полтора и ветре в 4–8 м/с это Re порядка 5·10⁵…10⁶,
# то есть как раз та область, где положение перехода решает всё.
NU_AIR = 1.48e-5        # кинематическая вязкость воздуха при 15 °C, м²/с
G = 9.80665
...
# Вылет бушприта от форштевня, м. Со слов владельца: бушприт выдвижной, 1.2 м.
# На плане парусности его нет вовсе — как нет и самого генакера, — поэтому это
# единственный источник, и он назван.
SPRIT_M = 1.2

# Отношения обводов генакера. ЭТО ОЦЕНКИ, а не обмер: чертежа паруса не
# существует, и вывести их неоткуда. Взяты типовыми для несимметричного паруса
# ходовых курсов и держатся тем, что при них площадь сходится с конструкторскими
# 27 м² (проверка в `_gennaker`).
#
#   задняя шкаторина короче передней — парус скручен вперёд;
#   средняя ширина к нижней — то, чем несимметричный парус отличается от
#     плоского треугольника: у reaching-паруса 0.7…0.8, у бегущего до 0.95.
GENNAKER_LEECH_K = 0.90
GENNAKER_GIRTH_K = 0.75
...
def main():
    ...
    doc = {
        ...
        "environment": {"rho_water": RHO_WATER, "rho_air": RHO_AIR, "g": G,
                        "nu_water": NU_WATER, "nu_air": NU_AIR},
        "mass": {
            "total_kg": mass,
        ...
        "foils": {
            ...
            "bulb": _bulb(keel["bulb"], keel["bulb_x_nose_mm"],
                          -calibrate.TARGET["draft_max_mm"] + keel["bulb"].radius),
...
def _gennaker(sp, forestay_stem_mm, mast_top_mm):
    """Генакер: точки крепления и обводы — ВЫВЕДЕННЫЕ, а не снятые.

    Ни паруса, ни бушприта на плане парусности нет. Есть три источника, и все
    три названы: площадь 27 м² от конструктора, вылет бушприта 1.2 м от
    владельца, топ мачты с обмера чертежа. Всё остальное здесь — вывод из них,
    и каждый шаг вывода записан.

    ГАЛС. На конце бушприта, в диаметральной плоскости, на высоте штагового
    узла: бушприт выдвигается от форштевня примерно горизонтально, и точка
    выхода у него там же, где стоит штаг. Высота — единственное, что здесь
    принято на глаз, и цена ошибки мала: сместить галс на десять сантиметров по
    высоте это полпроцента длины передней шкаторины.

    ФАЛ. На топе мачты, а не на хаундах. Это ВЫВОД ИЗ ПЛОЩАДИ, а не
    предположение: при подъёме на хаунды передняя шкаторина выходит 7.77 м, и
    чтобы получить 27 м², парусу нужна средняя ширина около 5.6 м — то есть
    шире, чем длина гика, и непропорционально низкий раздутый мешок. При
    подъёме на топ шкаторина 9.44 м, ширина выходит около 4.5 м, и пропорции
    становятся обычными для паруса такой площади.

    ОБВОДЫ. Несимметричный парус не рисуется в диаметральной плоскости, как
    грот со стакселем: он летит СБОКУ от лодки, и его очертание в пространстве
    зависит от того, как выбран шкот. Поэтому в пакет идут не координаты
    контура, а собственные размеры паруса — длины шкаторин и средняя ширина, —
    а место в пространстве симулятор строит сам по двум закреплённым точкам и
    длине шкота.

    ПЛОЩАДЬ считается по обмерной формуле для несимметричных парусов:

        S = (SLU + SLE)/2 · (SFL + 4·SHW)/6,

    где SLU и SLE — передняя и задняя шкаторины, SFL — нижняя, SHW — средняя
    ширина. Нижняя шкаторина здесь и подбирается: она единственная неизвестная,
    когда площадь задана.
    """
    tack = (forestay_stem_mm[0] + SPRIT_M * 1000.0, forestay_stem_mm[1])
    head = (mast_top_mm[0], mast_top_mm[1])
    slu = math.hypot(head[0] - tack[0], head[1] - tack[1]) / 1000.0
    sle = GENNAKER_LEECH_K * slu
    # S = (SLU+SLE)/2 · (SFL + 4·SHW)/6, SHW = k·SFL  ->  SFL из площади.
    sfl = 6.0 * SPINNAKER_M2 / ((slu + sle) / 2.0) / (1.0 + 4.0 * GENNAKER_GIRTH_K)
    shw = GENNAKER_GIRTH_K * sfl
    area = (slu + sle) / 2.0 * (sfl + 4.0 * shw) / 6.0
    assert abs(area - SPINNAKER_M2) < 1e-6, area
    # Обвод в ДП — ПРЕДСТАВЛЕНИЕ для полосок, а не форма летящего паруса.
    #
    # Симулятор режет парус на полоски по высоте и берёт хорду полоски как
    # разность передней и задней шкаторин на этой высоте. Генакер летит сбоку, и
    # его настоящее очертание в ДП не лежит; но ширина по высоте у него своя, и
    # именно её надо отдать полоскам. Поэтому задняя шкаторина здесь СЧИТАЕТСЯ
    # из ширины: leech(z) = luff(z) − ширина(z), а ширина идёт от нижней
    # шкаторины через среднюю к нулю у фала.
    #
    # Площадь такого обвода выходит меньше обмерной (24.4 против 27): обмерная
    # формула считает серп, а ломаная по трём точкам его не знает. Это не беда и
    # не подгонка: симулятор приводит сумму полосок к площади из пакета — ровно
    # так же, как делает это для грота со стакселем, у которых полоски не
    # достают до срезанных краешков.
    tk, hd = (tack[0] / 1000.0, tack[1] / 1000.0), (head[0] / 1000.0, head[1] / 1000.0)
    zmid = (tk[1] + hd[1]) / 2.0
    xmid = (tk[0] + hd[0]) / 2.0
    luff_pts = [[round(tk[0], 4), round(tk[1], 4)], [round(hd[0], 4), round(hd[1], 4)]]
    leech_pts = [[round(hd[0], 4), round(hd[1], 4)],
                 [round(xmid - shw, 4), round(zmid, 4)],
                 [round(tk[0] - sfl, 4), round(tk[1], 4)]]
    return {
        "tack_m": [round(tack[0] / 1000.0, 4), round(tack[1] / 1000.0, 4)],
        "head_m": [round(head[0] / 1000.0, 4), round(head[1] / 1000.0, 4)],
        "tack": [round(tk[0], 4), round(tk[1], 4)],
        "head": [round(hd[0], 4), round(hd[1], 4)],
        "head_aft": [round(hd[0], 4), round(hd[1], 4)],
        "clew": [round(tk[0] - sfl, 4), round(tk[1], 4)],
        "luff": luff_pts,
        "leech": leech_pts,
        "sprit_m": SPRIT_M,
        "luff_m": round(slu, 4),
        "leech_m": round(sle, 4),
        "foot_m": round(sfl, 4),
        "girth_mid_m": round(shw, 4),
        "area_m2": round(area, 3),
        "note": ("Выведен, а не обмерен: чертежа паруса нет. Источники — площадь "
                 "27 м² (ТТХ конструктора), вылет бушприта 1.2 м (владелец), топ "
                 "мачты (обмер чертежа). Подъём на топ выбран по площади: на "
                 "хаундах пропорции паруса выходят несуразными. Отношения "
                 "шкаторин и средней ширины — оценки, см. GENNAKER_*_K."),
    }
...
def _rig(sp, hull):
    ...
    return {
        ...
        # выдавала 33 м² там, где конструктор пишет 27.
        "spinnaker_area_m2": SPINNAKER_M2,
        "gennaker": _gennaker(sp, sp["forestay"]["stem_mm"], mast_top),
...
CREW_AREA_M2 = 0.52
CREW_CD = 1.1
# Экипаж втроём — 220 кг. Восемьдесят на человека было взято как круглое число
# для парусности, но с динамической плавучестью тот же вес стал садить лодку, и
# круглое число превратилось в тяжёлый экипаж: 240 кг на 590 кг корпуса. Для
# спортбота этого размера семьдесят три на человека ближе к делу.
CREW_MASS_EACH = 73.3
...
def _bulb(bulb, x_nose_mm, z_axis_mm):
    """Бульб: своё сопротивление, а не довесок к килю.

    Своего сопротивления у бульба в модели не было вовсе, и это не мелочь: у
    пера и руля оно идёт через профильный коэффициент крыла, отнесённый к
    площади в плане, а бульб — тело вращения. Ни площади в плане, ни
    коэффициента крыла у него нет, и он просто выпал — шесть ньютонов на
    лавировочном ходу.

    Надбавка на форму по Хёрнеру для тела вращения: 1 + 1.5·(d/L)^1.5 + 7·(d/L)³.
    При удлинении шесть это 1.13. Трение — ITTC-57 по длине бульба, а не по
    длине корпуса: у него своё число Рейнольдса, вчетверо меньше.

    Стык бульба с пером не в счёт: его зализывают, и у скруглённого Хёрнер даёт
    втрое меньше нескруглённого. Подъёмная сила на угле дрейфа тоже: у тела
    вращения она мала, а работу концевой шайбы бульб и так делает — она сидит в
    эффективном удлинении киля.
    """
    d = 2.0 * bulb.radius / 1000.0
    L = bulb.length / 1000.0
    tc = d / L
    return {
        "wetted_m2": round(bulb.wetted_mm2() / 1.0e6, 5),
        "length_m": round(L, 4),
        "diameter_m": round(d, 4),
        "form_factor": round(1.0 + 1.5 * tc ** 1.5 + 7.0 * tc ** 3, 4),
        "x_m": round((x_nose_mm - 0.45 * bulb.length) / 1000.0, 4),
        "z_centre_m": round(z_axis_mm / 1000.0, 4),
    }
