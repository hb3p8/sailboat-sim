...
# Остаточное сопротивление в долях веса как функция числа Фруда.
#
# Волновое сопротивление теперь УМЕЕТ считаться по обводам — интеграл Мичелла
# в src/sv20/wavemaking.py, с проверками, которые печатаются при сборке. Но в
# пакет идёт по-прежнему подобранная таблица, и вот почему.
#
# Форма у Мичелла оказалась похожей: отношение таблицы к расчёту держится в
# коридоре 0.45…0.64 на всём ходовом диапазоне. Если подставить расчёт и
# подобрать один множитель под наблюдение владельца (галфвинд 8.5 узла), якорь
# сходится — а лодка начинает лавировать через 72° вместо 82°. Разница вся в
# полосе Фруда 0.30…0.45: у Мичелла кривая там растёт круче, и лодке выгоднее
# приводиться, чем разгоняться.
#
# Лавировочный баланс на это разобран, и виноват в расхождении не он. Углы
# качества, посчитанные вдоль курса относительно воды, лежат в коридорах
# продувок и буксировок: у рига 9.5…11.9° (качество 4.7…6.0 при 4.5…6 в
# трубе), у подводной части 15.5…21.9° (у килевых лодок 14…22). Классическое
# тождество — угол между кажущимся ветром и курсом равен сумме этих двух —
# выполняется с точностью до градуса. Ни рангоут, ни киль тут не при чём.
#
# Чего не хватает — ВОЛНЕНИЯ. Сопротивление в пакете посчитано для тихой воды,
# волны в сцене нарисованы и на лодку не действуют, а настоящие 80…90°
# лавировочного угла — это угол на воде с волной. Волна штрафует именно
# приведение: на малом ходу лодке нечем сквозь неё проталкиваться.
#
# Разница между этой таблицей и расчётом ровно такой формы и есть: около 15%
# полного сопротивления на приводящемся конце, сходящие на нет с разгоном. В
# публикациях добавочное сопротивление на волнении для лодок такого размера в
# лавировку — 10…40%. То есть нарисованная таблица всё это время несла в себе
# волнение, слитое с волновым сопротивлением тихой воды.
#
# Разделить их — следующая работа: тихая вода расчётом (он уже есть), волнение
# отдельным слагаемым со своим состоянием моря. Тогда множитель Мичелла
# подбирается по якорю на гладкой воде, а остальные наблюдения владельца
# становятся проверками на волнении. Пока не разделено — таблица остаётся.
#
# Множитель 0.72 подобран 02.08.2026 по наблюдению владельца: галфвинд в 11.7
# узлах ветра у рига, экипаж на борту, парус настроен — около 8.5 узла.
RESIDUARY = [
    (0.15, 0.0006), (0.25, 0.0042), (0.35, 0.0162), (0.45, 0.0400),
    (0.55, 0.0648), (0.70, 0.0835), (0.85, 0.0900), (1.00, 0.0922),
    (1.20, 0.0904), (1.50, 0.0896), (2.00, 0.0936),
]
...
def resistance_curve(hull, lwl_mm, wetted_m2, mass_kg, v_max=9.0, n=45):
    ...
    Рядом с подобранным остаточным кладётся посчитанное по Мичеллу — не в
    работу, а для сверки: по нему видно, насколько подгонка расходится с
    расчётом и где именно.
    """
    lwl = lwl_mm / 1000.0
...
def main():
    ...
    doc = {
        ...
        "resistance": {
            ...
            "note": ("Трение — ITTC-57 с надбавкой на форму, это расчёт. "
                     "Остаточное — подобранная таблица; рядом в rw_michell_n "
                     "лежит волновое, посчитанное по обводам интегралом "
                     "Мичелла (sv20/wavemaking.py), для сверки. Почему в "
                     "работе всё ещё таблица — в комментарии к RESIDUARY."),
        },
        "foils": {
    ...
    print("  скорость      Rf     Rr подобр.   Rw Мичелл   отношение")
    for v in (2.0, 3.0, 4.0, 6.0, 8.0):
        row = min(r, key=lambda q: abs(q["v_ms"] - v))
        rw = row.get("rw_michell_n", 0.0)
        print("  %.1f м/с (%4.1f уз) %6.0f %10.0f %11.0f %10s"
              % (row["v_ms"], row["v_ms"] * 1.94384, row["rf_n"], row["rr_n"],
                 rw, ("%.2f" % (row["rr_n"] / rw)) if rw > 1 else "—"))
    ...
    return {
        ...
        "shroud": None if shroud is None else {
            ...
            "chainplate_y_m": round(shroud.get("chainplate_y_mm", 0.0)
                                    / 1000.0, 4)},
        # Корпус и экипаж в потоке. Мачты здесь больше нет: её сопротивление
        # считается по полоскам грота, где ему и место — на своей высоте, со
        # своим кренящим плечом и со своим местным ветром. Прежние 1.15 м²
        # включали мачту (8.7 м на 0.068 в среднем — это 0.59 м²) и приложены
        # были на высоте метра с небольшим, то есть кренила она вчетверо слабее,
        # чем на самом деле.
        "windage_area_m2": 0.56,
        "windage_cd": 0.85,
