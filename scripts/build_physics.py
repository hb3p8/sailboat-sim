...
#
# ЗАДНЯЯ ШКАТОРИНА КОРОЧЕ ПЕРЕДНЕЙ: парус скручен вперёд. 0.90 — оценка, и она
# помечена оценкой; на площадь она влияет слабо (входит средним с передней).
GENNAKER_LEECH_K = 0.90
GENNAKER_GIRTH_K = 0.85

# Пузо серпа задней шкаторины — в долях нижней шкаторины, в середине высоты.
#
# Считается, а не назначается. У треугольника с прямыми сторонами средняя ширина
# при здешней геометрии выходит РОВНО ПОЛОВИНОЙ нижней шкаторины (это следует из
# того, что галс вынесен вперёд, и проверяется в `_gennaker`). Обмерных 0.85
# прямыми сторонами не получить никак — недостающее и есть серп:
#
#     серп = (SHW/SFL − 0.5) · SFL
#
# Отсюда и берётся округлая задняя шкаторина, которой у паруса не может не быть.
GENNAKER_ROACH_K = GENNAKER_GIRTH_K - 0.5
...
def _gennaker(sp, forestay_stem_mm, mast_top_mm):
    ...
    # Задняя шкаторина — ОКРУГЛАЯ, и это не украшение. Прямыми сторонами
    # обмерной средней ширины не получить: у треугольника с этим галсом она
    # выходит ровно половиной нижней шкаторины (проверяется ниже), а по обмеру
    # нужно 0.85. Недостающее и есть серп, выгнутый в корму, — то, чем
    # несимметричный парус и отличается от плоского треугольника.
    #
    # Форма серпа — синусом по высоте: ноль у фала, ноль у шкотового угла,
    # наибольшее в середине. Точек девять: с тремя выходил излом, и парус читался
    # четырёхугольным.
    clew = (tk[0] - sfl, tk[1])
    span = hd[1] - tk[1]
    roach = GENNAKER_ROACH_K * sfl
    tri_mid = (hd[0] + clew[0]) / 2.0
    assert abs(((tk[0] + hd[0]) / 2.0 - tri_mid) - sfl / 2.0) < 1e-9, "средняя ширина треугольника"
    # Задняя шкаторина начинается не в точке, а от заднего края фаловой дощечки.
    head_aft_x = hd[0] - GENNAKER_HEAD_W
    leech_pts = []
    for k in range(9):
        f = k / 8.0
        z = hd[1] - f * span                       # сверху вниз, от фала к шкоту
        x_line = head_aft_x + (clew[0] - head_aft_x) * f
        leech_pts.append([round(x_line - roach * math.sin(math.pi * f), 4), round(z, 4)])
    ...
    # к диаметральной, потравил — ушёл наружу и вперёд. Значит настройка у
    # генакера не угол, а ДЛИНА, и пределы её задаёт геометрия, а не вкус.
    def sheet_len(theta):
        cx = tk[0] - sfl * math.cos(theta)
        cy = sfl * math.sin(theta)
        return math.hypot(cx - lead[0], cy - lead[1], tk[1] - lead[2])

    return {
        "sheet_lead_m": lead,
        # Короче не бывает: шкотовый угол в ДП, прямо за галсом.
        "sheet_min_m": round(sheet_len(0.0), 3),
        # Дальше ста десяти градусов парус уже не несёт, а полощет: за этим
        # пределом травить нечего.
        "sheet_max_m": round(sheet_len(math.radians(110)), 3),
