...
const SAIL_KEEP = 0.2;
// Каким ядром пелена видится ПАРУСУ. Панель по хорде без малого метр, и
// разрешать полю на ней особенности мельче — значит требовать от сетки того,
// чего в ней нет.
const SAIL_BLUR = 0.3;
const NEAR = [0, 0, 0];
...
export class Rig {
  ...
  wakeDownwash(b, lat) {
    ...
    w.hold = true;
    // Размазывание берётся с рига, а не с константы: батарея Вагнера гоняет его
    // по значениям, и это единственный способ показать числом, что оно и лечит
    // разнос, и стоит нестационарности.
    const blur = this.sailBlur != null ? this.sailBlur : SAIL_BLUR;
    w.rcFloor = blur * blur;
    w.edges(); w.pack();
  ...
  stepWake(b, dt) {
    ...
    w.hold = false; w.rcFloor = 0; w.spaceCore(); w.edges(); w.pack();
  }
}
