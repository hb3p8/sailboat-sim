...
const BENCH_PARTS = [
  ...
  // Застройка своим узлом, и снять её можно на ходу. Лес так не снимается — он
  // печётся в один буфер с землёй, — но застройка устроена так же: вертикальные
  // стенки у самой воды, перекрывающие друг друга. Если цена берега в заливке,
  // видно будет и по ней одной.
  ['без застройки', () => {
    for (const p of terrainParts) if (/застройка/.test(p.o.name || '')) p.o.visible = false;
  }],
