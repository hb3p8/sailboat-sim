...
export class Buoyancy {
  ...
  // `hump` — собственная волна корпуса ({amp, k, xf}) или null. Вода у станции
  // x стоит на ζ(x) = amp·cos(k·(xf − x)): гребень у форштевня, впадина там,
  // куда её ставит дисперсия. Каждый шпангоут срезается СВОИМ уровнем — из
  // этого сами собой выходят и просадка кормы, и дифферент на нос кверху, и
  // сдвиг центра величины: никаких отдельных формул момента не нужно.
  at(zc, phi, th, hump) {
    ...
    for (let i = 0; i < n; i++) {
      const zeta = hump ? hump.amp * Math.cos(hump.k * (hump.xf - xs[i])) : 0;
      this.station(i, ny, nz, zeta - zc - nx * xs[i]);
    }
