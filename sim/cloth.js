...
export class Cloth {
  ...
  advance(b, h, side, env) {
    ...
    for (let r = 0; r < CLOTH_ROWS; r++) {
      ...
      // ДАВЛЕНИЕ НА ПАНЕЛЬ — ПО КУТТЕ-ЖУКОВСКОМУ, а не нормированное на единицу.
      //
      // Прежде здесь стояло `qd·(q[k]/peak)`, то есть форма нагрузки по хорде,
      // поделённая на свой же максимум и умноженная на полный скоростной напор.
      // Перепад давления выходил ровно 0.5·ρ·V² в пике ВСЕГДА — при любом угле
      // атаки, при любой циркуляции. Это следящая сила постоянной величины,
      // всегда по нормали к деформируемой поверхности, — классический
      // неконсервативный источник ФЛАТТЕРА, и он им и оказался.
      //
      // Мера (docs/wake.md): при полностью замороженной лодке полотно
      // колебалось само, размах пуза 0.0798 с периодом 1.46 с, ровно с 28-й
      // секунды до 60-й. Собственное затухание ткани съедает колебание за
      // период более чем в тысячу раз (по нормали e^-2.6, вязкое PBD e^-8.8) —
      // значит петля закачивала ровно столько же. И порог по скорости был на
      // месте, как положено флаттеру: на 6 м/с колебания нет (0.0000), на
      // 10 м/с есть.
      //
      // Физика, которой не хватало, — зависимость ВЕЛИЧИНЫ нагрузки от угла.
      // Настоящий парус, повернувшись из потока, разгружается, и это его
      // аэродинамическая жёсткость; нормировка на пик её убирала начисто.
      // Перепад берётся из той же циркуляции, которой панель уже вошла в силу:
      // Δp = ρ·V·Γ/Δx, а Δx = хорда/NCHORD — одна реализация одной вещи.
      //
      // Хорда здесь ЛЕТЯЩАЯ, та самая, которую положила ткань (Б3). Вырожденной
      // она быть не может — у верхней полоски и та под метр, — но ноль в
      // знаменателе стоит исключить явно, а не полагаться на это.
      const kq = live && q && g.chord > 1e-3
        ? this.rhoAir * g.ve * NCHORD / g.chord : 0;
      ...
      for (let c = 0; c < CLOTH_COLS; c++) {
        ...
        const pr = kq * q[kp] * sgn * this.area[i];
