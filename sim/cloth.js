...
// три угла — галс и фал закреплены, шкотовый висит на шкоте, — и давление из
// решётки. Форму находит позиционная динамика (Müller и др., 2007): предсказали
...
//
// В СТРАНИЦУ МОДУЛЬ ПОКА ВКЛЕИВАЕТСЯ, А НЕ ЗОВЁТСЯ, и это не недосмотр. Сборщик
// требует, чтобы каждый файл `sim/*.js` был в списке вклейки, иначе он ругается;
// а звать ткань из отрисовки рано: натянутая на нынешний обвод генакера, она
// сходится в мешок, и причина не в ней (`tests/cloth.test.mjs`, заголовок).
// Подменить картинку мешком значило бы ухудшить её.
//
// Цену будущей подмены стоит назвать заранее. Когда ткань пойдёт в отрисовку,
// нарисованное полотно и посчитанные панели РАЗОЙДУТСЯ: ткань находит хорды
// сама, а решётка берёт их из шкота со скруткой. До сих пор они совпадали по
...
import { edgeFn, sailSagAt, STRIPS, NCHORD, gennakerSheetLen } from './aero.js';
...
export class Cloth {
  ...
  step(b, dt) {
    ...
    const calc = rig.stripCalc, strips = rig.strips, base = this.si * STRIPS;
  ...
  project(b, side) {
    ...
    const lead = gen && gen.sheet_lead_m;
    const sheet = gen ? gennakerSheetLen(b.o, gen) : 0;
    for (let it = 0; it < ITER; it++) {
      for (let k = 0; k < M; k++) {
        ...
        if (d <= rest[k] || d < 1e-9) continue;
        const g = ck[k] * (d - rest[k]) / d / s;
        p[a] += wa * g * dx; p[a + 1] += wa * g * dy; p[a + 2] += wa * g * dz;
        p[c] -= wb * g * dx; p[c + 1] -= wb * g * dy; p[c + 2] -= wb * g * dz;
      }
      // Шкот. Связь тоже односторонняя: выбранный шкот держит шкотовый угол на
      // дуге, отданный не держит ничего. Обух — точка лодки, и сторона у него
      // та же, на которой стоит парус.
      if (lead && sheet > 0) {
        const k = this.clew * 3;
        const lx = lead[0], ly = lead[1] * side, lz = lead[2];
