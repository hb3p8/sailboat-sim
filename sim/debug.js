...
//
// Линий шестьдесят, а не двадцать шесть, и это стало возможно ровно тогда, когда
// из ядра решётки убрали аллокации: точка стоит 1.63 мкс, линия в 49 шагов — 80
// мкс, шестьдесят линий — 4.8 мс при бюджете кадра в 33 мс. До того одни только
// двадцать шесть занимали десять миллисекунд и считались через одиннадцать
// кадров на двенадцатый.
const FLOW_LINES = 60, FLOW_STEPS = 48, FLOW_DS = 0.275;
// Через сколько кадров пересчитываются линии. См. main.js, где это и считается.
const FLOW_EVERY = 3;
let flowTick = 0;
const FLOW_SEED = 4.5;        // насколько выше по потоку начинать, м

// Засев тремя рядами по высоте. Верхний — отдельная история: он сажается
// вплотную под головку паруса, потому что сходящая с топа концевая нить это
// единственный настоящий вихрь в модели, и посмотреть на него можно только
// вблизи. Двумя прежними рядами (на 1.05 и 0.45 высоты ЦП, то есть 4.3 м и 1.9
// м) до головки на девяти метрах не доставала ни одна линия — не «попадала
// случайно», а не попадала вовсе.
//
// Верхний ряд и разлетается уже остальных: чтобы линия обошла вокруг нити, она
// должна пройти рядом с ней, а разбросанная на три метра в сторону пройдёт мимо.
const FLOW_ROWS = 3, FLOW_COLS = FLOW_LINES / FLOW_ROWS;
const FLOW_TOP_GAP = 0.25;    // насколько ниже головки сажать верхний ряд, м
const FLOW_TOP_HALF = 1.2;    // полуразлёт верхнего ряда поперёк потока, м
const FLOW_HALF = 3.0;        // полуразлёт остальных
const FLOW_SIDES = 5;
const FLOW_R = 0.028;         // радиус трубки, м
...
flowGroup.add(flow);
// Прозрачность призрака привязана к числу линий, а не выбрана на глаз: она
// копится, и то, что при двадцати шести читалось как намёк на поток за парусом,
// при шестидесяти закрасило лодку сплошной пеленой. 0.16 × 26/60 ≈ 0.07;
// поставлено 0.09 — чуть плотнее счёта, потому что линии не ложатся друг на
// друга ровно.
...
function updateFlow() {
  ...
  // Верхняя точка решётки — оттуда и сходит концевая нить. Берётся с панелей, а
  // не из пакета: панели уже стоят с твистом и с нынешним пузом, а головка в
  // пакете — это чертёж в покое.
  // Берётся и поперечная координата тоже, и это не мелочь: решётка стоит уже с
  // креном, и на двадцати градусах головка на девяти метрах уходит вбок на три с
  // половиной метра. Верхний ряд, посаженный по диаметральной, прошёл бы мимо
  // нити вовсе — замер это и показал: с центровкой по диаметральной лучшая линия
  // делала 0.40 оборота, с центровкой по самой головке — 0.94.
  let topZ = -Infinity, topX = rigX, topY = 0;
  for (let j = 0; j < lat.n; j++) {
    const pn = lat.panels[j];
    if (pn.a[2] > topZ) { topZ = pn.a[2]; topX = pn.a[0]; topY = pn.a[1]; }
    if (pn.b[2] > topZ) { topZ = pn.b[2]; topX = pn.b[0]; topY = pn.b[1]; }
  }
  let at = 0;
  for (let i = 0; i < FLOW_LINES; i++) {
    ...
    // лодку.
    const row = i % FLOW_ROWS, col = (i - row) / FLOW_ROWS;
    const top = row === 0;
    const half = top ? FLOW_TOP_HALF : FLOW_HALF;
    const lat0 = (col / (FLOW_COLS - 1) - 0.5) * 2 * half;
    // Верхний ряд целится в саму головку, а не в ЦП: полтора метра разницы по
    // длине лодки на разлёте в метр двадцать решают, пройдёт линия у нити или
    // мимо неё.
    const cx = top ? topX : rigX, cy = top ? topY : 0;
    const h = top ? topZ - FLOW_TOP_GAP : (row === 1 ? rigZ * 1.05 : rigZ * 0.45);
    let x = cx - ux * FLOW_SEED + px * lat0;
    let y = cy - uy * FLOW_SEED + py * lat0;
    let z = h;
    for (let k = 0; k <= FLOW_STEPS; k++) {
      // Поле — то же, по которому модель считает силы: связанные вихри БЕЗ
      // прямых лучей плюс сама пелена. Раньше здесь стояли лучи, и линии
      // показывали поле модели, которой уже нет.
      //
      // Пелена спрашивается в мировых осях, а линии интегрируются в осях
      // корпуса, отсюда переводы. Стоит это тринадцать миллисекунд на
      // обновление против двух с половиной у одной решётки — поэтому линии и
      // считаются не каждый кадр (FLOW_EVERY).
      lat.induced(x, y, z, ux, uy, 0, true, flowV, wf);
      let wx = 0, wy = 0, wz = 0;
      if (wf) {
        const X = boat.x + x * bc - y * bs, Y = boat.y + x * bs + y * bc;
        boat.rig.wake.induced(X, Y, z, true, wakeV);
        wx = wakeV[0] * bc + wakeV[1] * bs;
        wy = -wakeV[0] * bs + wakeV[1] * bc;
        wz = wakeV[2];
      }
      const vx = aw.x + flowV[0] + wx, vy = aw.y + flowV[1] + wy, vz = flowV[2] + wz;
