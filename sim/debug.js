...
function updateFlow() {
  ...
  for (let i = 0; i < FLOW_LINES; i++) {
    ...
    let z = h;
    for (let k = 0; k <= FLOW_STEPS; k++) {
      // Поле берётся с ПРЯМЫМИ сходящими лучами, хотя силы модель считает по
      // свободной пелене. Это осознанная замена, и вот её цена.
      //
      // Честное поле — это связанные вихри плюс сама пелена, а она стоит тысячу
      // с лишним отрезков. Шестьдесят линий по сорок шагов это две с половиной
      // тысячи точек, то есть три миллиона отрезков на обновление — двадцать
      // миллисекунд там, где на весь вид отведено пять. Прямой луч же и есть
      // пелена, выпрямленная по потоку, и наводит он ту же величину с точностью
      // до пятой: батарея меряет это прямо — 0.42 м/с против 0.52 у пелены в
      // контрольных точках (tests/wake.test.mjs).
      //
      // То есть линии показывают, куда идёт воздух, с точностью до сворачивания
      // пелены. Где это важно — есть вид самой пелены.
