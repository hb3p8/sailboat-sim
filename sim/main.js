...
// Плотность пикселей. На телефоне ниже, и это первый по дешевизне рычаг из всех.
//
// Вода занимает почти весь кадр, и её цена — это цена пикселя, помноженная на их
// число. У телефона плотность три, то есть при потолке в два кадр считается в
// четыре раза большем разрешении, чем показывает CSS. Полтора вместо двух — это
// вдвое меньше пикселей, а в руке разницы почти не видно: экран мелкий, и вода
// на нём и так рябая.
//
// Замер, из-за которого это сделано: на плато (docs/perf.md, §3.4) видеокарта
// тратила 16.5 мс из 17.6-миллисекундного кадра.
const PIXELS_MAX = MOBILE_UI ? 1.5 : 2;
renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, PIXELS_MAX));
