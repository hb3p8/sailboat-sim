...
// Свободная пелена на телефоне в силы НЕ входит.
//
// Стоит она там девять с половиной миллисекунд на кадр из семнадцати с
// половиной (docs/perf.md, §3.4) — то есть больше половины процессорной части
// кадра ради одной величины: неустановившегося отклика паруса.
//
// Чем платим, и это не «ничем». Без пелены скос считают прямые сходящие лучи, а
// запаздывание — подобранный фильтр `alphaLag`, который её и заменял до неё.
// Установившийся ход при этом ТОЖЕ отличается, хоть и немного: замер по шести
// курсам в шесть метров в секунду даёт от −3.9% на галфвинде до +2.5% в
// бейдевинд, крен — до полуградуса. Больше всего теряется там, где пелена и
// заводилась: на перекладке, порыве и повороте отклик приходит фильтром, а не
// сходящим листом.
//
// Размен принят по замеру: сорок кадров в секунду на плато против тридцати с
// небольшим. На настольной машине пелена как была, так и осталась.
if (MOBILE_UI) boat.o.wakeForces = false;
