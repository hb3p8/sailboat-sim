...
//
// В СИЛЫ ЭТО ИДЁТ (шаг Б3): по этому же полотну решётка строит панели генакера,
// так что нарисованное и посчитанное — одна поверхность, а не две.
// Ткань живёт в риге (`Boat.hoistCloth`): она же считает форму панелей, значит
// рисовать надо ровно её, а не вторую такую же.
function gennakerCloth() { return boat.rig.cloth; }
...
function shapeSails(side, dt) {
  ...
  // Шага ткани здесь нет: её двигает риг вместе с силами, в физическом темпе.
  // Полотно и панели решётки теперь одна и та же поверхность — до шага Б3 они
  // расходились, и это было записано ценой.
