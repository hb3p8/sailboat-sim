...
function updateCrew() {
  ...
  // Поперёк фигурки двигает плечо, а вот РАЗВЁРНУТЫ они по борту паруса: ноги
  // должны смотреть внутрь лодки, а знак плеча для этого не годится — у оси он
  // вырождается в ноль, и фигурки принимались бы крутиться. Борт берётся тем же
  // выражением, каким рисуется парус, с обратным знаком: экипаж на наветренном
  // борту по определению есть «не там, где парус».
  ...
  for (let i = 0; i < crewFigs.length; i++) {
    ...
    f.position.set(bodyPointLocalX(CREW_X[i]), bodyPointLocalY(sheer[1]),
                   bodyPointLocalZ(-arm));
    // Доворот такой, чтобы ноги смотрели внутрь лодки. Поворот three вокруг Y
    // уменьшает угол в плоскости XZ, отсюда знак: ноги надо привести к -side.
    f.rotation.y = crewLegs + side * Math.PI / 2;
  }
