...
// множитель один, и применяется он одинаково. При ±18% порывистости язык даёт
// сорок процентов прибавки к ряби, и это ровно та разница, которую на воде и
// видно.
// Само поле порывов берётся здесь один раз на стадию и дальше идёт и в
// амплитуду ряби, и в цвет: два вызова — две выборки из текстуры за один и тот
// же ответ.
const seaRipple = svGust(seaWorldXY).mul(seaGust);
const seaGustVert = svGust(seaRefXY).mul(seaGust).add(1.0).clamp(0.2, 2.0);
const seaGustFrag = seaRipple.add(1.0).clamp(0.2, 2.0);
