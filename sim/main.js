...
const STREAK_W = 0.04;
// Станции вдоль струи: доля длины и полуширина в долях максимальной. Их шесть,
// а не три: тремя выходит ромб с изломом на плечах, а нужна капля — округлая
// голова и длинный сходящий хвост. Заодно станции — это точки ломаной, и по ним
// же струя гнётся в потоке (см. ниже), так что лишние две дают и форму, и изгиб.
const STREAK_ST = [[0.00, 0.00], [0.04, 0.62], [0.13, 1.00],
                   [0.32, 0.80], [0.62, 0.44], [1.00, 0.00]];
...
function updateStreaks(ix, iy, dt) {
  ...
  const amp = Math.max(0, Math.min(1, (boat.o.windSpeed - 1.2) / 5));
  ...
  for (let i = 0; i < STREAK_N; i++) {
    ...
    for (let k = 0; k < STREAK_ST.length; k++) {
      ...
      if (k) {
        // Шаг назад по потоку на разницу долей длины, вдоль МЕСТНОГО направления.
        const seg = L * (t - STREAK_ST[k - 1][0]);
        const f = streakFlow(cx, cz, st.y, ux, uy, aw, cpsi, spsi);
        sdx = f.dx; sdz = f.dz;
        cx -= sdx * seg; cz -= sdz * seg;
      }
