...
//
// У грота со стакселем передняя шкаторина прибита по всей длине, и построчная
// сборка — хорда из обвода, поворот на вынос, выгиб пузом — даёт им верную
// поверхность. У генакера впереди нет ничего: он висит на трёх углах, поперёк
// хорд у него своя кривизна, и построчно она не получается никак — выходит
// цилиндр (план §Б0.3). Ткань считает её сама: сетка узлов, нерастяжимость,
// давление из решётки (`sim/cloth.js`).
//
// В СИЛЫ ЭТО НЕ ИДЁТ. Решётка по-прежнему живёт на полосках, и отпечаток от
// ткани не меняется ни на байт. Цена названа: нарисованное полотно и посчитанные
// панели теперь расходятся там, где ткань находит хорду сама, — сводит их шаг Б3.
let genCloth = null;
function gennakerCloth() {
  const sails = boat.rig.sails;
  if (sails.length < 3) { genCloth = null; return null; }
  if (!genCloth || genCloth.sail !== sails[2]) genCloth = new Cloth(sails[2], 2);
  return genCloth;
}
...
function shapeSails(side, dt) {
  ...
  // Ткань генакера — шаг в её собственном времени. Шаг кадра, а не физики: она
  // ничего не отдаёт назад, и считать её чаще картинки незачем.
  const cloth = gennakerCloth();
  if (cloth) cloth.step(boat, dtFrame);
  ...
  boat.rig.sails.forEach((sail, k) => {
    ...
    for (let r = 0; r < SAIL_ROWS; r++) {
      ...
      const nx = -uy, ny = ux;
      // Генакер — по ткани, остальные построчно. Заполаскивание одно и то же:
      // оно и там и там наложение поверх готовой поверхности.
      const byCloth = sail.gennaker && cloth;
      const cn = byCloth ? cloth.rowNormal(r) : null;
      for (let c = 0; c < SAIL_COLS; c++) {
        ...
        let bow = byCloth ? 0 : camber * chord * Math.sin(Math.PI * u);
        let shade = 1;
        if (flap && t < lf) {
        ...
        if (byCloth) {
          // Оси ткани — рига: (вперёд, вбок, вверх). В сцене вертикаль это Y, а
          // борт меняет знак: (x, y, z) -> (x, z, −y), как в axes.js.
          cloth.sample(r, t, clothPt);
          a[i] = clothPt[0] + bow * cn[0];
          a[i + 1] = clothPt[2] + bow * cn[2];
          a[i + 2] = -(clothPt[1] + bow * cn[1]);
        } else {
