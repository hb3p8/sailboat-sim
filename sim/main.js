...
function frame() {
  ...
  // Свободная пелена считается только в своих двух видах: она стоит 18 мс на шаг
  // физики (замер scripts, шаг без неё — 0.23 мс), а в силы пока не входит
  // вовсе. Включать её всегда значило бы платить полкадра за то, чего не видно.
  //
  // Было девять, стало восемнадцать: кольца с поперечными рёбрами и хвостом
  // удвоили число звеньев. Перед тем как пелена войдёт в силы, за эту цифру
  // придётся взяться — там она будет платиться каждый шаг, а не в отладке.
