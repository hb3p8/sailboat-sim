...
// Признак «телефона» (`MOBILE_UI`) объявлен в util.js — от него зависит не
// только этот интерфейс, но и плотность пикселей, карта теней и пелена в силах.
// Сам признак живёт в util.js: от него зависит не только интерфейс.
...
// Вертикальные ползунки — это шкоты, и растут они ВНИЗ: низ — выбран, верх —
// потравлен. Так ходит рука. Выбрать шкот значит тянуть его к себе и вниз, через
// стопор или лебёдку; потравить — отпустить, и его выбирает вверх сам парус.
// Первый заход был устроен наоборот, и жест выходил обратным настоящему: чтобы
// добрать, палец приходилось вести туда, куда на лодке шкот сам уходит.
function dragBar(el, vertical, onMove) {
  let id = null;
  const at = e => {
    const r = el.getBoundingClientRect();
    const t = vertical ? (e.clientY - r.top) / r.height
...
//
// И это именно РУМПЕЛЬ, а не «поворот»: ползунок вправо — румпель на правый
// борт, а лодка идёт ВЛЕВО. Так на лодке с румпелем и есть, и подписан орган
// тоже румпелем. Знак переворачивается ровно в одном месте — там, где команда
// уходит в модель (gameApply), — а весь интерфейс, и ручка, и шкала, и цифра,
// живёт в одну сторону с рукой. Держать переворот в двух местах значит однажды
// перевернуть только одно из них и получить прибор, спорящий сам с собой.
//
// На клавишах остаётся как было: стрелка вправо — нос вправо. Стрелки не
// румпель, они направление, и трогать их этот переворот не должен.
let gameTiller = 0;              // −1 … +1, плюс — румпель на правый борт
...
// Скорости, с какими клавиши двигают органы. Взяты не с потолка: у шкотов это те
// же 32°/с, что и на отладочной раскладке, переведённые в доли своих размахов, —
// клавиша обязана работать одинаково, на какой бы раскладке её ни нажали. У
// румпеля — скорость удерживаемой стрелки интерфейса, 45°/с.
const GAME_KEY_MAIN = 32 / (90 - 7);
const GAME_KEY_JIB = 32 / 90;
const GAME_KEY_TILLER = 45 / TILLER_MAX;

// Клавиатура на игровой раскладке.
//
// Без этого её тут просто нет: readControls честно разбирает клавиши и пишет
// команды в лодку, а следом идёт gameApply и затирает их положением органов —
// руль и шкоты возвращаются туда, где стоят ползунки. Со стороны это выглядит
// как «клавиши не работают», хотя работают они прекрасно, только их результат
// живёт полкадра.
//
// Чинится не отключением одного из двух, а тем, что клавиша двигает ТОТ ЖЕ
// ОРГАН, за который тянут пальцем. Тогда источник один, спорить нечему, и
// ползунок под клавишей едет сам — видно, что нажатие дошло.
//
// Нужно это на телефоне, где клавиатуры нет, ровно затем же, зачем сам `?ui=game`
// на ноутбуке: игровую раскладку правят с настольной машины, и вести лодку при
// этом удобнее клавишами, чем возить мышью по нарисованному румпелю.
function gameKeys(dt) {
  const kL = keys.ArrowLeft || keys.KeyA, kR = keys.ArrowRight || keys.KeyD;
  // Стрелка — направление НОСА, и это её смысл везде. Румпель ходит в другую
  // сторону, оттого и минус: «вправо» значит румпель на левый борт.
  if (kL !== kR) tillerSet(gameTiller + (kL ? 1 : -1) * GAME_KEY_TILLER * dt);
  const set = (k, v) => sheetSet(k, Math.max(0, Math.min(1, gameSheet[k] + v)));
  if (keys.ArrowUp || keys.KeyW) set('main', GAME_KEY_MAIN * dt);
  if (keys.ArrowDown || keys.KeyS) set('main', -GAME_KEY_MAIN * dt);
  if (keys.KeyR) set('jib', GAME_KEY_JIB * dt);
  if (keys.KeyF) set('jib', -GAME_KEY_JIB * dt);
}
...
function gameApply(o, dt) {
  ...
  gameDt = dt;
  gameKeys(dt);
  // ЕДИНСТВЕННОЕ МЕСТО, где положение румпеля становится командой рулю, и
  // единственное, где меняется знак: румпель вправо — нос влево.
...
function gameHud(t) {
  ...
  // Под автоматом перо читается с лодки и переворачивается в положение румпеля:
  // прибор подписан румпелем и обязан показывать румпель, чей бы он ни был.
...
function gameBar(el, t, vertical, at) {
  ...
  const r = el.getBoundingClientRect();
  if (vertical) {
    // Ручка едет вниз по мере выбирания, заливка идёт за ней сверху: закрашено
    // ровно то, что уже выбрано. Заливка привязана к верху ползунка в разметке —
    // здесь ей остаётся только высота.
    const k = knob.offsetHeight || 26;
    knob.style.top = (t * (r.height - k)) + 'px';
