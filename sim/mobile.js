...
// медленно уходить с курса, не понимая почему. Порог узкий, в один градус:
// зона шире отнимает у рулевого власть над малыми углами, а ими и правят на
// длинном галсе.
//
...
// двигают руль вовсе. Так однажды и вышло — 2.8° шага против 3° зоны.
const TILLER_SNAP = 1 / TILLER_MAX;
...
function gameHud(t) {
  ...
  // Под авторулевым число показывает ФАКТИЧЕСКОЕ отклонение пера, а не команду:
  // команды у рулевого в этот момент нет вовсе, руль кладёт автомат, и
  // единственное осмысленное число здесь — то, на сколько он его положил. Заодно
  // по нему видно, сколько автомат работает: держит курс в полградуса руля или
  // борется с волной на пятнадцати.
  //
  // На руках, наоборот, показывается КОМАНДА. Перо доходит до неё не мгновенно —
  // скорость перекладки ограничивает сама лодка, — и показывать догоняющее перо
  // значило бы, что орган не слушается: положил румпель, а число ползёт.
  const tillDeg = autopilot ? boat.o.rudder / D : gameTiller * TILLER_MAX;
  gameEl.till.textContent = Math.abs(tillDeg) < 0.5 ? '0°'
    : Math.abs(tillDeg).toFixed(0) + '°' + (tillDeg > 0 ? ' пр' : ' лв');
  ...
  gameBar(document.getElementById('gtill'),
          tillDeg / TILLER_MAX * 0.5 + 0.5, false);
}
