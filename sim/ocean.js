...
//
// Вынесено сюда, а не оставлено в main.js, ровно из-за поворота. Это место, где
// легко перепутать борт со знаком, и ошибка вышла бы тихой: лодка кренилась бы
// не в ту сторону только на некоторых курсах. Здесь его можно проверить
// тестом — что одна и та же вода даёт одни и те же наклоны в мире на любом
// курсе, — и он проверяет (tests/ocean.test.mjs).
function oceanPlaneFit(h, psi, halfL, halfB, out) {
  const c = Math.cos(psi), s = Math.sin(psi);
  const along = (h[1] - h[2]) / (2 * halfL);
  const across = (h[3] - h[4]) / (2 * halfB);
  out.z = (h[0] + h[1] + h[2] + h[3] + h[4]) / 5;
...
export { Ocean, oceanBand, oceanPlaneFit, OCEAN_TILES, OCEAN_N, OCEAN_LOOP, OCEAN_CHOP, OCEAN_FOAM_J };
