...
class Ocean {
  ...
  // Горстка точек, в которых поверхность нужна процессору: под лодкой, под
  // знаками на воде. Читается это асинхронно и приходит с задержкой в кадр —
  // на воде с периодом в пару секунд такая задержка меньше миллиметра.
  _buildProbe() {
    this.probeCount = 8;
    ...
    this.kProbe = Fn(() => {
      ...
      this.probeOut.element(instanceIndex).assign(vec4(d.z, s.x, s.y, s.w));
...
export { Ocean, oceanBand, OCEAN_TILES, OCEAN_N, OCEAN_LOOP, OCEAN_CHOP, OCEAN_FOAM_J };
