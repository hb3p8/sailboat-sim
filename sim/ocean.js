...
class Ocean {
  ...
  constructor(renderer, seed = 1) {
    ...
    // Сколько кадров ещё держать прошлый прогон. Первый считается сразу.
    this._skip = 0;
  ...
  //
  // НЕ КАЖДЫЙ КАДР. Волна — самое дорогое, что есть на телефоне (21.9 мс на
  // кадр, docs/perf.md), а меняется она медленно: за шестидесятую долю секунды
  // поверхность сдвигается на миллиметры. Считается она поэтому раз в
  // OCEAN_EVERY кадров, а между прогонами держится последняя — картинка при
  // этом не дёргается, потому что смещения берутся из тех же текстур, а не
  // пересчитываются в вершинах.
  //
  // Пробы под лодкой идут тем же пакетом и потому обновляются так же редко.
  // Физике это ничего не стоит: шаг у неё 1/30 с, период волны — секунды, и
  // запаздывание в кадр-другой на её фоне не видно. Тем же пакетом они и
  // остаются: разделять их значило бы гонять БПФ ради проб.
  step(t) {
    if (!this.ready) return;
    this.uTime.value = t - Math.floor(t / OCEAN_LOOP) * OCEAN_LOOP;
    if (this._skip > 0) { this._skip--; return; }
    this._skip = OCEAN_EVERY - 1;
    // Счётчик прогонов — для прибора. Метка времени с видеокарты меряет ОДИН
    // пакет, а не кадр: пакет идёт не каждый кадр, и «столько стоит счёт»
    // получается только поделив на то, сколько кадров прошло между пакетами.
    this.runs++;
    ...
    //
    // Пакет собран ЗАРАНЕЕ (`kBatch`): здесь горячий путь, и `[...a, b]` в нём
    // означал бы новый массив каждый кадр.
    this.renderer.compute(this.kBatch);
    if (!this._probeBusy) {
