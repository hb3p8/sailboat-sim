...
// Сторона решётки каскада. 128 — это 16 тысяч волн на каскад и семь ступеней
// бабочек.
//
// Было 256, и на настольной машине разницы в цене не чувствовалось: все
// вычислительные проходы вместе стоили 0.2…1.1 мс. На телефоне те же проходы
// показали 21.9 мс — больше, чем вся отрисовка (docs/perf.md, §3), и «считать
// дёшево» оказалось свойством настольной видеокарты, а не модели. Половина
// стороны — это вчетверо меньше элементов на каскад и на ступень бабочек меньше.
//
// Чем платим: самая короткая разрешаемая волна вдвое длиннее. У мелкого каскада
// плитка 3.6 м, значит вместо 2.8 см на узел выходит 5.6 — мелкой ряби на воде
// становится меньше, и её недостачу добирает шейдерная рябь, которая и так
// рисуется поверх.
const OCEAN_N = 128;
const OCEAN_LOG2N = 7;

// Раз во сколько кадров считается волна. Два — это 15…30 герц обновления
// поверхности при 30…60 кадрах: волна с периодом в пару секунд за это время
// сдвигается на сантиметры, и на глаз пауза не читается. Замер, ради которого
// это заведено, — docs/perf.md, §3.
const OCEAN_EVERY = 2;
...
class Ocean {
  ...
  constructor(renderer, seed = 1) {
    ...
    this._buildProbe();
    // Готовый пакет кадра: ядра волны и следом проба под лодкой. Собирается
    ...
    this.ready = false;
    this.runs = 0;               // сколько раз волна СЧИТАЛАСЬ (не сколько кадров)
    this.every = OCEAN_EVERY;    // и как редко — для шапки профиля
    this.gridN = OCEAN_N;
  }
  ...
  //
  // РАБОТА РАЗЛОЖЕНА ПО КАДРАМ, а не считается реже. Разница не в среднем — оно
  // то же самое, — а в том, чем задаётся частота кадров.
  //
  // Раньше пакет шёл целиком раз в OCEAN_EVERY кадров, и на телефоне это дало
  // такую картину: отрисовка 13.5 мс каждый кадр плюс пакет 12.8 через раз, то
  // есть кадры чередовались 26.3 и 13.5 при среднем 19.9. А фактический кадр
  // был 25.1 мс — потому что частоту задаёт ТЯЖЁЛЫЙ кадр, а лёгкий потом ждёт.
  // Платили по худшему из двух вместо среднего.
  //
  // Теперь каждый кадр считает свою долю пакета. Свежесть при этом не меняется
  // ни на сколько: полный обход по-прежнему укладывается в OCEAN_EVERY кадров.
  // Меняется только ровность цены.
  //
  // Резать можно посередине цепочки, и это не случайность. Внутри каскада
  // проходы связаны (спектр -> БПФ по строкам -> БПФ по столбцам -> сборка ->
  // нормали), но ВЫХОДНЫЕ текстуры пишут только два последних. Остановились на
  // середине — выход держит прошлый полный ответ, а промежуточные буферы у
  // каскада свои. Каскады же между собой независимы вовсе.
  //
  // Плата: каскады оказываются на слегка разном времени — один успел прокрутить
  // фазы на кадр позже другого. Это разные масштабы волны, они и так независимы,
  // а расхождение в кадр даёт миллиметры.
  //
  // Пробы под лодкой идут последними в том же пакете и обновляются раз в полный
  // обход, как и раньше. Физике это ничего не стоит: шаг у неё 1/30 с, период
  // волны — секунды.
  step(t) {
    if (!this.ready) return;
    this.uTime.value = t - Math.floor(t / OCEAN_LOOP) * OCEAN_LOOP;
    // Счётчик кадров, в которых счёт ВООБЩЕ БЫЛ: по нему прибор переводит метку
    // времени в цену кадра. Теперь он растёт каждый кадр, и «счёт» с «пакетом»
    // на экране обязаны сойтись — это и есть признак того, что цена ровная.
    this.runs++;
    // Одной отправкой: внутри прохода WebGPU держит порядок и видимость записей,
    // а на каждый вызов compute() приходится своя отправка очереди.
    this.renderer.compute(this.kSlices[this._phase]);
    const last = this._phase === this.kSlices.length - 1;
    this._phase = last ? 0 : this._phase + 1;
    // Проба читается после доли, в которой она посчиталась, — то есть раз в
    // полный обход.
    if (last && !this._probeBusy) {
