...
function perfSample(now) {
  return {
    ...
    //
    // СОСТАВ РИГА — сюда же, и в каждый отсчёт, а не в заголовок. Генакер
    // поднимают и убирают ПО ХОДУ, а он это плюс шесть полосок к двенадцати и
    // плюс семь нитей пелены к четырнадцати — то есть цена шага меняется в
    // полтора-два раза посреди сессии. Без этого два прогона на одной сборке
    // разошлись в 1.4 раза, и отличить «стало быстрее» от «был другой риг» было
    // нечем: в ленте про риг не стояло ничего.
    //
    // `узлов` — сколько узлов пелены реально посчитано. Она набирается первые
    // секунды, и ранние отсчёты дешевле не от кода, а оттого, что считать ещё
    // нечего.
    полосок: boat && boat.rig ? boat.rig.strips.length : 0,
    нитей: boat && boat.rig && boat.rig.wake ? boat.rig.wake.fil : 0,
    узлов: boat && boat.rig && boat.rig.wake ? boat.rig.wake.n : 0,
