...
export class Boat {
  ...
  reset() {
    ...
    this.rigSide = 1;                // на каком борту стоит парус
    this.t = 0;
  }
  ...
  sailForces(aw) {
    ...
    // С какого борта стоит парус — состояние лодки, а не мгновенный отсчёт по
    // флюгеру: парус вынесен на один борт целиком, в середине не ломается и от
    // колебания ветра туда-сюда не прыгает. Перекидывается, только когда ветер
    // уверенно зашёл на другую сторону.
    const want = aw.angle > 0 ? 1 : -1;
    if (want !== this.rigSide &&
    ...
    for (let i = 0; i < this.strips.length; i++) {
      ...
      const chordDir = rigSide * (Math.PI - set);
