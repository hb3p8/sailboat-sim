...
import { membraneCamber, slackOf, fillFactor, luffFraction, luffFactor, sectionLift, capLift, liftCeiling } from './membrane.js';
    ...
    if (awSpeed > 0.2) {
      ...
      for (let it = 0; it <= NLIN; it++) {
        ...
        for (let i = 0; i < NS; i++) {
          ...
          //
          // Смятой ткани решётка сама по себе не видит: полощущему стакселю
          // на нулевом угле атаки она всё равно считает cl около единицы —
          // столько даёт жёсткое сечение с десятипроцентным пузом. Мягкому
          // сечению на нулевом угле пузо держать нечем, и сечение это знает, а
          // решётка нет. От этого полощущий стаксель наводил на топ грота
          // подпор, и на отданных шкотах грот заполаскивал снизу вверх, а не
          // сверху вниз, как положено при твисте.
          //
          // Считается наполнение один раз и подставляется в последний проход,
          // без итерирования. Итерировать нельзя: наполнение само считается по
          // нагрузке, поправка его уменьшает, и риг за три шага складывается —
          // проверено.
          if (it === NLIN - 1) {
            const sSail = sailSign[i < STRIPS ? 0 : 1];
            const mem = membraneCamber(q, g.slack);
            const dead = Math.min(1, Math.max(
              luffFraction(q, sSail),
              1 - luffFactor(Math.abs(g.alpha), mem.ideal)));
            camS[i] = (1 - dead) * this.strips[i].mastFill;
          }
