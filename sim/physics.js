...
export class Boat {
  ...
  constructor(pack, terrain, opts) {
    ...
    this.o = Object.assign({
      ...
      //
      // Считается теперь устойчиво и даёт осмысленный ответ: ход в бейдевинде
      // 2.61 -> 2.80 м/с, дрейф меньше, циркуляция держится в 11-13 на любой
      // частоте шага. Но меняется и НЕСТАЦИОНАРНЫЙ отклик, а он в модели пока
      // назначен фильтром (`alphaLag`, функция Вагнера звеном). Поднятый
      // выключатель даёт по батарее «половина прироста сразу» 106% вместо 50%:
      // отклик выходит мгновенным, да ещё с перелётом. То есть пелена свою
      // нестационарность даёт, но не ту.
      //
      // Разбираться с этим — III.2.c, где `alphaLag` и должен уйти, а отклик
      // сверяться с задачей Вагнера. Поднимать выключатель до этого значит
      // променять подогнанное запаздывание на неизвестно какое.
      //
      // Он же — единственный способ спросить «а сколько это даёт» на одном и
      // том же состоянии, не пересобирая модель. Сам он поднимает и `freeWake`.
      wakeForces: false,
