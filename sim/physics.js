...
export class Boat {
  ...
  constructor(pack, terrain, opts) {
    ...
    this.o = Object.assign({
      ...
      // около четырёх миллисекунд на шаг, а в силы пока не входит вовсе
      // (docs/flow-plan.md, III.2.a). Симулятор включает её в отладочном виде
      // потока, где её и видно.
      freeWake: false,
      // Пелена ВХОДИТ В СИЛЫ. Выключатель опущен, и осталось для этого одно.
      //
      // Всё прочее сошлось: счёт устойчив и не зависит от частоты шага,
      // чувствительность к шкоту на месте (оптимум на 16°, как и у прямых
      // лучей), запаздывание пелена даёт сама, обе подпорки против разноса
      // оказались не нужны и убраны.
      //
      // Не сходится ПЕРВЫЙ отсчёт после мгновенной перекладки: циркуляция
      // меняется на 1.14 установившейся вместо вагнеровских 0.5, и батарея
      // запаздывания в wind.test.mjs валится на нём двумя проверками. Причина
      // известна и описана в docs/wake.md: разгонный вихрь сидит в v·dt позади
      // кромки, а не на ней, и его противодействие ослаблено. Лечится тем, что
      // первое звено пелены делают короче шага сноса (Кац и Плоткин).
      //
      // Выключатель оставлен и после того, как будет поднят: им меряется
      // разница на одном и том же состоянии. Оговорка к сравнению на лету: у
      ...
      // два прогона с одинаковым прогревом, а не переключение посреди хода.
      wakeForces: false,
