...
// Плечо ОТКРЕНИВАЮЩЕГО экипажа, м: насколько вбок от диаметральной уходит его
// центр тяжести, когда `crewHike` доведён до упора.
//
// Прежде здесь стоял метр — полуширина по линии борта в кокпите, снятая с сетки
// корпуса (`CREW_SHEER` в sim/main.js). Это плечо СИДЯЩЕГО: столько выносит
// вес человек, сидящий на борту с ногами внутрь. Но `crewHike = ±1` означает не
// «сидит на борту», а «откренивает до упора», а у откренивающего ноги под
// ремнями и корпус вынесен наружу: центр тяжести уходит за линию борта ещё на
// сорок сантиметров с небольшим. Метр и четыре десятых — это и есть он.
//
// Цена подмены была спрятана ошибкой знака в моменте крена от крыльев (ниже,
// `foilHeel`): кренящее плечо выходило на треть короче настоящего, и заниженное
// откренивание с ним сходилось. Порознь не работает ни одно: с верным знаком и
// сидящим плечом лодка в одиннадцать метров в секунду не выходит из левентика
// и теряет узел хода на лавировке.
const CREW_ARM = 1.4;
...
export class Boat {
  ...
  step(dt) {
    ...
    // КАЧКА в местной скорости крыла, и это не поправка второго порядка.
    //
    // Момент крена у киля и руля есть (`foilHeel` ниже), а движения, которому
    // этот момент сопряжён, в их скорости не было вовсе: приходили только снос
    // и рыскание. Работа тогда считается по одной половине пары, и энергия за
    // цикл качки не сходится — модель имеет право качаться сама.
    //
    // Величина простая. Точка корпуса на высоте `zb` (в осях лодки) при
    // скорости крена `p_` движется поперёк ДП со скоростью `−p_·(zb − z_ЦТ)`;
    // косинусы крена сокращаются между двумя плечами и в ответ не входят.
    // Отсюда же и знак: киль СНИЖЕ центра тяжести, и на качке он идёт в
    // противоход мачте.
    //
    // Это и есть главное демпфирование качки на ходу: у несущего крыла оно
    // растёт с ходом линейно (подъёмная сила ~ V²·α, а α ~ p_/V), тогда как
    // глухой множитель ниже от хода не зависит вовсе.
    const cgz0 = P.mass.cg_m[2];
    const keelRz = keel.z_centre_m - cgz0, rudRz = rud.z_centre_m - cgz0;
    const kf = foilForce(env, keel, this.u,
                              this.v + this.r * (keel.x_m - cgx0)
                              - this.p_ * keelRz, 0, 0);
    ...
    // Подъёмная сила днища и её плечи — один раз на шаг: она входит и в
    // восстанавливающий момент, и в дифферент, и во всплытие.
    const planeLift = pl && hyd ? pl.lift : 0;
    const ply = hyd ? hyd.cby : 0, plz = hyd ? hyd.cbz - cgz : 0;
    ...
    if (hyd) {
      ...
      if (planeLift > 0) righting += planeLift * (ply * nz - plz * ny);
    ...
    // Момент крена от крыла — ТОЙ ЖЕ формулой, что у полоски паруса и у
    // ветрового сопротивления: `−(z − z_ЦТ)·F`. Прежде здесь стоял обратный
    // знак, и это была не договорённость, а описка: у ТЕХ ЖЕ крыльев момент
    // ДИФФЕРЕНТА ниже в этом же файле собран правильно —
    // `mth += -(keel.z_centre_m - cgz) * keelFx`.
    //
    // Что знак неверен, видно и статикой. У пары «парус и киль» при равных и
    // противоположных силах плечо обязано равняться расстоянию ОТ ЦЕНТРА
    // ПАРУСНОСТИ ДО ЦЕНТРА БОКОВОГО СОПРОТИВЛЕНИЯ. У SV20 это 4.93 м, а с
    // прежним знаком выходило 3.41 м — на треть меньше.
    //
    // Сила берётся ДО разложения по крену (`kf.fy`, не `keelSide`): у момента
    // два плеча, поперечное и вертикальное, и косинус с синусом между ними
    // сокращаются ровно так же, как в скорости выше. Держать здесь `keelSide`
    // значило бы посчитать только одно из двух.
    const foilHeel = -(kf.fy * keelRz + rf.fy * rudRz);
    // БОКОВАЯ СИЛА КОРПУСА тоже кренит, и её здесь не было совсем. Поперечное
    // течение вокруг накренённого днища приложено на половине смоченной
    // глубины — той самой, которую `hullLateral` уже считает и отдаёт наружу.
    // Сила эта горизонтальная, вертикальной доли у неё нет, поэтому форма та
    // же, что у ветрового сопротивления.
    const hullHeel = -(-0.5 * hull.depth - m.cg_m[2]) * hull.fy;
    ...
    const dp = (sailHeel + windHeel + foilHeel + hullHeel +
                righting + hikeMoment + damp) / ix;
    ...
    if (hyd) {
      ...
      // Плечи подъёмной силы днища. Продольное — своё (центр давления у
      // Савицкого), поперечное и вертикальное — те же, что у плавучести.
      //
      // Поперечное плечо здесь не мелочь, а половина всего дела. Днище несёт там,
      // где оно смочено, а у накренённой лодки смочена и нагружена подветренная
      // половина — центр давления уходит туда же, и этим глиссирующий корпус
      // держится на ровном киле. Приложить силу по диаметральной значит отнять у
      // корпуса остойчивость, ничего не дав взамен: вентиляция уносит кормовую
      // часть ватерлинии вместе с её вкладом в восстанавливающий момент, а днище
      // его не возвращает. Замер этого стоил: крен на 13 м/с выходил 33° против
      // 12° без вентиляции.
      //
      // Центр берётся у СМОЧЕННОГО ОБЪЁМА, а не назначается: та же геометрия,
      // которой считается плавучесть, и второй раз её считать незачем.
      ...
      // в плавный набор.
      if (planeLift > 0) mth += planeLift * ((pl.xcp - cgx) * nz - plz * nx);
