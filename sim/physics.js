...
export class Boat {
  ...
  constructor(pack, terrain, opts) {
    ...
    this.o = Object.assign({
      ...
      //
      // Поднятый, он даёт разошедшийся счёт, и мешает ровно одно: свободная
      // пелена проходит СКВОЗЬ паруса. На лавировке след стакселя идёт по
      // гроту, узлы подходят к контрольным точкам на три-девятнадцать
      // сантиметров, и наведённая ими нормальная скорость доходит до 15 м/с при
      // кажущемся ветре 8. Решать систему с такой правой частью бессмысленно.
      //
      // Это не неожиданность: в плане это риск №4 части III («пелена и парус»),
      // и разумный ответ там же — гасить или отодвигать узлы, попавшие в парус.
      // Пока этого нет, выключатель лежит.
      //
