...
export class Boat {
  ...
  constructor(pack, terrain, opts) {
    ...
    this.o = Object.assign({
      ...
      freeWake: false,
      // Пелена ВХОДИТ В СИЛЫ. Выключатель опущен, и вот почему.
      //
      // Само по себе всё работает: счёт устойчив, отклик идёт по Вагнеру (0.66
      // на второй полухорде против классических 0.67), эталон осмысленный — в
      // бейдевинде тяга 341 -> 390 Н, боковая -1102 -> -1005, дрейф -0.312 ->
      // -0.281 рад, то есть сила паруса повернулась вперёд. Из девятнадцати
      // батарей восемнадцать проходят.
      //
      // Не проходит три проверки, и все три — настоящие:
      //
      //  1. На ПЕРВОМ шаге после мгновенной перекладки циркуляция прыгает в 1.2
      //     установившейся вместо 0.5. Дальше всё правильно, но батарея
      //     запаздывания мерит именно этот шаг.
      //  2. Ход перестал отличать перебранный шкот (12°) от правильного (16°) —
      //     5.54 узла на обоих.
      //  3. Цена: 10-12 мс на шаг против 0.25. Треть реального времени на одну
      //     физику, и батареи гоняются десятками минут вместо секунд.
      //
      // Пока это не разобрано, выключатель лежит. Он же — единственный способ
      // спросить «а сколько это даёт» на одном и том же состоянии, не
      // пересобирая модель. Сам он поднимает и `freeWake`.
      wakeForces: false,
