...
export function telemetryOf(b, f) {
  return {
    ...
    // Истинный ветер У ЛОДКИ — со всем, что с ним по дороге случилось:
    // профилем по высоте, порывом и тенью берега. Это и есть то, что чувствуют
    // паруса. Сверять картинку надо с ним, а не с положением ползунка: ползунок
    // задаёт ветер на стандартных десяти метрах и в чистом поле.
    twsKn: f.aw.ws * 1.94384,         // истинный ветер на высоте ЦП
    twsMs: f.aw.ws,
    // Само поле порывов в точке лодки, от −1 до +1: та же величина, которой
    // вода красит языки. По ней картинка и сверяется с силами.
    gustG: b.gustG || 0,
