...
export class Lattice {
  constructor(n) {
    ...
    // он кладётся сюда снаружи. Нули, когда пелена в силы не введена.
  ...
  // `tailW` — доля прямых сходящих лучей у каждой панели. Нужна, когда пелену
  ...
  // не матрицей.
  //
  // Выключатель, а не вес на панель, и это проверено замером. Полоска за срывом
  // сбрасывает в пелену не всю циркуляцию (`Rig.wakeShed`), и напрашивается
  // отдать остаток прямому лучу — тогда контур замкнут при любой доле. Так и
  // было сделано, и так делать НЕЛЬЗЯ: на полных курсах поток идёт от кормы, то
  // есть луч уходит ВПЕРЁД и утыкается в стаксель, тогда как свободный лист от
  // него отгибается. На повороте через фордевинд циркуляция стакселя от этого
  // уходила в 428 против 25. Подробности и числа — docs/wake.md, §5.10.
  // `tailW` — доля ПРЯМОГО ХВОСТА у каждой панели, от нуля до единицы.
  //
  // Раньше здесь стоял логический `noTail`: со свободной пеленой хвосты
  // убирались целиком, потому что их место занимает свежее кольцо. Пока сход
  // полный, это верно. Как только сход стал ЧАСТИЧНЫМ (`wakeShed` между нулём и
  // единицей), выбор «есть хвост или нет» перестал закрывать контур: связанный
  // вихрь и обе ножки несут полную Γ, а замыкающее кольцо только `f·Γ`, и на
  // задней кромке остаются свободные концы силой `(1−f)·Γ`. Вихревая линия
  // обрывается в воздухе — это нарушение Гельмгольца, и разносится ровно оно
  // (docs/wake.md, §9.3 и §5.12).
  //
  // Теперь замыканий ДВА, и они складываются в единицу: доля `f` замыкается
  // свободным кольцом (`nearWake` берёт её тем же множителем), доля `1−f` —
  // прямым хвостом отсюда. Связанный вихрь и ножки при этом одни, общие; сумма
  // двух замыканий несёт полную Γ, и оборванных концов не остаётся ни при каком
  // `f`. `tailW = null` — прежнее поведение с полным хвостом.
  build(ux, uy, uz, self, ground, tailW) {
  ...
  // добавляет себе сама.
  // `fade` — по доле на панель: сколько от решённой циркуляции панель ДЕЙСТВИТЕЛЬНО
  // держит. Ноль означает сорванную полоску, и тогда её вихрей в поле нет вовсе.
  //
  // Заведено ради Кельвина. Сход в пелену гасится за срывом (`Rig.wakeShed`), а
  // связанная циркуляция до сих пор оставалась полной — то есть парус считался
  // сорванным для пелены и пристёгнутым для поля. Для грота со стакселем такие
  // углы редкость; для генакера это рабочий диапазон, и там это вылезло сразу:
  // в пелену сходит ноль, а узлы её летят семьдесят метров в секунду от поля
  // связанных вихрей.
  packBound(ground, fade) {
    ...
    for (let mir = 0; mir < (ground ? 2 : 1); mir++) {
      ...
      for (let j = 0; j < n; j++) {
        const g = fade ? this.gamma[j] * fade[j] : this.gamma[j];
        if (!g) continue;
        const p = P[j], a = p.a, b = p.b, ta = p.ta, tb = p.tb;
  ...
  // `on` — какие панели ещё в решётке. Выключенная получает не малый вес, а
  // тождество Γ = 0: единица на диагонали, ноль в правой части. Условие
  // непротекания для неё не ставится вовсе — потенциальное течение о сорванной
  // поверхности ничего сказать не может, и притворяться, что может, дороже, чем
  // промолчать. Влияние на остальных при этом пропадает само: с нулевой Γ
  // столбец ничего не даёт.
  // `pre` — ПРЕДПИСАННАЯ циркуляция вынутых панелей.
  //
  // Вынутая панель получает строку-тождество, и раньше в правой части там
  // стоял ноль: парус исчезал из вихревой системы совсем. Это и было место, где
  // модель заводила ВТОРУЮ реализацию одной вещи — силу такой полоске всё равно
  // считала поляра сечения, то есть число «сколько она заворачивает воздух» у
  // модели было, и оно выбрасывалось (docs/gennaker-planing-plan.md, §4.13.4).
  //
  // Теперь туда кладётся Γ из поляры. Связь получается ОДНОСТОРОННЕЙ, и это
  // ровно то, чего требует ревью: столбец вынутой панели остаётся в уравнениях
  // ОСТАЛЬНЫХ, поэтому чужие паруса её скос видят, а её собственная строка —
  // тождество, поэтому её угол атаки от её же циркуляции не зависит и петля
  // Γ → пелена → скос → Γ на ней не замыкается.
  solveLinear(rhs, on, pre) {
    const n = this.n, g = this.gamma;
    const M = this._m || (this._m = new Float64Array(n * (n + 1)));
    for (let i = 0; i < n; i++) {
      if (on && !on[i]) {
        for (let j = 0; j < n; j++) M[i * (n + 1) + j] = i === j ? 1 : 0;
        M[i * (n + 1) + n] = pre ? pre[i] : 0;
        continue;
      }
...
//
// Она ВХОДИТ В СИЛЫ: место прямых лучей в матрице влияния занял этот лист.
// Сошедшие кольца известны и приходят правой частью, свежее кольцо неизвестно и
// входит в матрицу тремя отрезками. За срывом полоска перестаёт сбрасывать лист
// (`Rig.wakeShed`), и тогда сходящей завихренности у неё не остаётся вовсе —
// вместе со скосом, который модель там и так не считает (docs/wake.md, §5.10).
...
export class FreeWake {
  ...
  // каждой. Двадцать четыре узла при тридцати герцах это 0.8 секунды подробной
  // истории: узел сходит каждый шаг. Больше стоит дорого — самоиндукция растёт
  // как квадрат длины цепочки, и длину подбирали замером (docs/wake.md, §4.2).
  //
  // Дальний конец не обрывается: он продолжен полубесконечным хвостом вдоль
  // потока, каким тот был при сходе последнего ряда. Оборванная нить наводит не
  // то, что тянущаяся до бесконечности, и на отклике это видно сразу.
  constructor(fil, len) {
    this.fil = fil;
    this.len = len;
    ...
    this.warm = false;   // ближайший шаг считать целиком: придержанному верить нельзя
    // Массивы, которые СТАРЕЮТ ВМЕСТЕ С УЗЛАМИ. Узел каждый шаг переезжает из
    // слота i в слот i+1, и всё, что про него посчитано и придержано до
    // следующего раза, обязано переезжать с ним — иначе придержанное достанется
    // соседу. Сюда пелена кладёт своё поле, а решётка (`Rig.stepWake`) — своё.
    this.aged = [];
  ...
  // Сброс. Обнулить `n` мало: придержанные скорости остаются от прошлой жизни,
  // и первые шаги новой лодка едет по чужому полю. Проверялось повторным
  // использованием лодки после `reset()` против свежей.
  // Сброс. Обнулить `n` мало: придержанные скорости остаются от прошлой жизни.
  // Разогревочный шаг здесь НЕ нужен и его тут нарочно нет: пелена начинается с
  // пустого места, каждый узел рождается молодым и первые `NEAR_EXACT` шагов
  // считается точно, — то есть придержанному просто неоткуда взяться старым.
  //
  // Симметрия здесь важнее краткости. Пелена заводится при первом шаге, и в
  // момент сброса лодки её может ещё не быть вовсе; поставь `clear()` разогрев —
  // и сброшенная лодка пошла бы иначе, чем свежая, ровно на этот один шаг.
  // Именно так и было, и поймано это сравнением со свежей.
  clear() {
    this.n = 0;
    this.selN = -1;
    for (const a of this.aged) a.fill(0);
  }

  // А вот ЗДЕСЬ разогрев нужен. Узлы пришли готовыми и всех возрастов —
  // восстановлены из записи, — а придержанных скоростей у них нет ни одной.
  // Ближайший шаг считает всех.
  invalidate() {
    for (const a of this.aged) a.fill(0);
    this.selN = -1;
    this.warm = true;
  ...
  field(qx, qy, qz, np, ox, oy, oz) {
    ...
    for (let j = 0; j < np; j++) { ox[j] = 0; oy[j] = 0; oz[j] = 0; }
    if (N < 2) return;
    // Цикл по рёбрам — в кернеле (kernel/biot.c), если он собрался. Ответ там
    // ПОБИТОВО тот же: вектор идёт по точкам, порядок сложения по рёбрам не
    // меняется, слияние в FMA запрещено флагом сборки. Проверяется равенством,
    // а не близостью, — tests/kernel.test.mjs.
    //
  ...
  // Полубесконечные хвосты. Отдельным проходом после рёбер — и порядок этот
  // существенный: кернел добавляет к уже посчитанному, а не считает заново.
  fieldTails(qx, qy, qz, np, ox, oy, oz) {
    const L = this.len, F = this.fil, N = this.n;
    // Зеркало от воды — как и у конечных рёбер в `pack`, только там оно
    // выключается признаком `ground`, а здесь его когда-то забыли спросить.
    // Стоило это доверия к стенду: крыло в свободном потоке лежит в плоскости
    // z = 0, зеркальный хвост там совпадает с настоящим и вычитается ровно —
    // то есть в самоиндукции стенда хвостов не оставалось вовсе, тогда как
    ...
    // складывает их в том же порядке (kernel/biot.c, `tails_at`). Знак уносится в силу: `x + ((-g)*y)`
  ...
  selfInduce() {
    ...
    if (!this.sx || this.sx.length < F * L) {
      ...
      this.aged.push(this.sx, this.sy, this.sz);
      // `sel` здесь НЕ заводится, и это не экономия строки. Заводит его
      // `pickSlice`, и зовётся он РАНЬШЕ — в начале шага. Строка `this.sel =
      // new Int32Array(F * L)` на этом месте затирала только что выбранные узлы
      // нулями, и весь набор точек схлопывался в нулевой узел. Случалось это
      // ровно один раз за жизнь лодки — на первом шаге, где считается своё поле,
      // — и потому пряталось: свежая лодка после этого шла иначе, чем
      // повторно использованная, у которой массив был заведён в прошлой жизни.
    }
    // Плотная раскладка `f * n + i` живёт РОВНО один вызов поля: поле берёт
    // точки подряд, а в `x`, `y`, `z` у каждой нити свой ряд длиной `len` и
    // между рядами дыры, пока пелена не набралась. Хранится же посчитанное по
    // месту узла — `f * len + i`. Смешивать эти два индекса нельзя: `n` растёт
    // каждый шаг, и придержанное значение оказалось бы под адресом другого узла.
    if (this.selN < 0) {
      ...
      this.field(this.qx, this.qy, this.qz, F * N, this.tx, this.ty, this.tz);
      for (let f = 0; f < F; f++) {
        for (let i = 0; i < N; i++) {
          const j = f * N + i, k = f * L + i;
          this.sx[k] = this.tx[j]; this.sy[k] = this.ty[j]; this.sz[k] = this.tz[j];
        }
      }
      return;
    }
    ...
    for (let q = 0; q < m; q++) {
      const k = this.sel[q];
      this.sx[k] = this.tx[q]; this.sy[k] = this.ty[q]; this.sz[k] = this.tz[q];
    }
  }
  ...
  //
  // НАБОР СЛОТОВ НЕПОДВИЖЕН, и это главное здесь. Считаются слоты 0, K, 2K…
  // (плюс `NEAR_EXACT` молодых всегда), а по этому неподвижному гребню узлы
  // ПРОЕЗЖАЮТ САМИ: каждый шаг узел стареет на слот, и раз в K шагов попадает на
  // считаемый. Ровно этого от деления и хотелось.
  //
  // Сначала здесь была бегущая фаза `ph = phase++ % K`, и она всё сводила на
  // нет. Фаза вращалась с той же скоростью, с какой стареют узлы, и они друг
  // друга гасили: если узел выбран сейчас (i % K == ph), то на следующем шаге
  // он в слоте i+1 при фазе ph+1 — и выбран снова. Свидетель прямо на это и
  // указал: узлы, рождённые на шагах 0 и 3, считались КАЖДЫЙ шаг, а рождённые
  // на 1, 2, 4, 5 — только первые четыре шага жизни (пока их держал
  // `NEAR_EXACT`) и дальше замирали навсегда, храня скорость восьмисотмиллисе-
  // кундной давности. То есть треть узлов считалась точно и без всякой
  // экономии, а две трети не обновлялись вовсе.
  pickSlice() {
    const L = this.len, F = this.fil, N = this.n;
    const K = Math.max(1, this.slice | 0);
    if (!this.sel || this.sel.length < F * L) this.sel = new Int32Array(F * L);
    if (K === 1 || this.warm) { this.warm = false; this.selN = -1; return; }
    let m = 0;
    for (let f = 0; f < F; f++) {
      for (let i = 0; i < N; i++) {
        if (i >= NEAR_EXACT && (i % K) !== 0) continue;
        this.sel[m++] = f * L + i;
      }
    }
    this.selN = m;
  ...
  step(dt, seed, vel, gring) {
    ...
    for (let f = 0; f < this.fil; f++) {
      ...
      for (let i = 0; i < this.n; i++) {
        const k = b + i;
        vel(this.x[k], this.y[k], this.z[k], v, k);
        if (this.n > 1) {
          this.vx[k] = v[0] + this.sx[k];
          this.vy[k] = v[1] + this.sy[k];
          this.vz[k] = v[2] + this.sz[k];
        } else {
    ...
    for (let f = 0; f < this.fil; f++) {
      ...
      for (let i = this.n - 1; i >= 0; i--) {
        ...
        if (i + 1 < L) {
          ...
          // Придержанное едет вместе с узлом: оно про него, а не про слот.
          // Условие по РЕЖИМУ, а не по текущему шагу: разогревочный точный шаг
          // (после сброса и после восстановления из записи) тоже стареет, и
          // если его не сдвинуть, следующий делёный шаг прочтёт всё на слот
          // назад.
          if (this.slice > 1)
            for (let a = 0; a < this.aged.length; a++) {
              const A = this.aged[a];
              A[k + 1] = A[k];
            }
        }
