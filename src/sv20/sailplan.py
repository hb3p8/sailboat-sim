...
def upwind_area_m2(features):
    """Лавировочная парусность: грот со стакселем по снятым обводам.

    Отдельная функция, потому что этим числом меряется не только риг: по нему
    же назначается площадь пера руля. Пока плана парусности не было, туда шли
    круглые 25 кв.м с сайта магазина, и перо получалось процентов на девять
    больше нужного — разница между 25 и обмерными 22.85 целиком уходила в него.

    Если план парусности почему-то не снят, берётся штамп самого чертежа: он
    ближе к обмеру, чем ТТХ магазина.
    """
    sp = (features or {}).get("sail_plan")
    return sp["area_total_m2"] if sp else SAIL_AREA_DRAWN_M2
