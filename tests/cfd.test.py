...
# Почти нулевая компонента мерится длиной вектора, а не собой. Проверяется не
# послабление, а то, что послабление НЕ ОСЛЕПИЛО меру: числа взяты прямо из
# пробы знаков (Mz — 4% длины вектора момента).
probe_f, probe_m = (-6.0008, -8.9743, -0.5469), (0.6325, -0.9155, -0.0485)
noise_f, noise_m = (-5.9999, 8.9743, -0.5541), (-0.6305, -0.9166, 0.0457)
res = ax.symmetry_residual((probe_f, probe_m), (noise_f, noise_m))
check("сеточный шум у мелкой компоненты не считается расхождением",
      res["Mz"] < 0.05, "Mz %.3f" % res["Mz"])
# А вот ошибка знака у САМОЙ МЕЛКОЙ компоненты обязана остаться видной: ради
# неё проверка и заведена, и мера, которая её пропустит, бесполезна.
flip_m = (noise_m[0], noise_m[1], -noise_m[2])
res = ax.symmetry_residual((probe_f, probe_m), (noise_f, flip_m))
check("ошибка знака у самой мелкой компоненты видна с большим запасом",
      res["Mz"] > 0.5, "Mz %.3f против допуска 0.05" % res["Mz"])
# И перестановка двух компонент — тем более.
swap_m = (noise_m[0], noise_m[2], noise_m[1])
res = ax.symmetry_residual((probe_f, probe_m), (noise_f, swap_m))
check("перестановка двух компонент момента видна",
      max(res["My"], res["Mz"]) > 0.5,
      "My %.3f, Mz %.3f" % (res["My"], res["Mz"]))

# --- окно усреднения не схлопывается ------------------------------------------
#
# Проверяется не удобство, а отказ. Прежний откат на две последние точки
# однажды выдал «сошедшееся» Cl 0.917 с нулевым дрейфом, снятое с двух
# соседних итераций посреди переходного процесса (§13.25). Среднее по двум
# точкам выглядит достовернее честного «не сошлось», и потому опаснее.
t_short = np.linspace(0.0, 869.0, 870)
try:
    fx.window(t_short, start=5200.0)
    check("окно за пределами расчёта отвергается", False, "приняло")
except ValueError as e:
    check("окно за пределами расчёта отвергается", "не набрало точек" in str(e))
try:
    fx.window(np.linspace(0.0, 100.0, 101), start=98.5)
    check("окно из трёх точек отвергается", False, "приняло")
except ValueError:
    check("окно из трёх точек отвергается", True)
idx, start = fx.window(np.linspace(0.0, 100.0, 101), start=50.0)
check("здоровое окно принимается", len(idx) > 40, "%d точек" % len(idx))
...
try:
    ...
    # набегающий поток в связанных осях идёт в сторону убывающего x. Обращённое
    # сечение — это острый нос и тупая корма, оно срывается сразу, и именно так
    # были посчитаны и NACA 0012 на десяти градусах, и первое сечение генакера,
    # у которого пузо якобы снижало подъёмную силу вчетверо.
    for cam in (0.0, 0.185):
        up, lo = geo.sail_section(cam, 0.45, 3.9, 0.015, n=40)
        check("носок сечения смотрит в поток: пузо %.3f" % cam,
              up[0, 0] > up[-1, 0],
              "носок x = %.3f, хвост x = %.3f" % (up[0, 0], up[-1, 0]))
        if cam > 0:
            mid = 0.5 * (up[:, 1] + lo[:, 1])
            # Положительное пузо обязано выпирать туда же, куда смотрит
            # подъёмная сила при положительном угле, то есть в −y.
            check("положительное пузо выпирает в сторону подъёмной силы",
                  mid[np.argmax(np.abs(mid))] < 0,
                  "%.3f" % mid[np.argmax(np.abs(mid))])
    naca = geo.naca_symmetric(0.12, 1.0, 60)
    check("носок NACA смотрит в поток",
          naca[0, 0] > naca[:, 0].min() + 0.5,
          "первая точка x = %.3f" % naca[0, 0])
...
try:
    ...
    # Аспект фоновой ячейки у плоского случая. Он не меняется при сгущении:
    # `snappyHexMesh` делит ячейку во всех направлениях сразу, и отношение
    # «в плоскости к поперёк» остаётся тем же, каким было в blockMesh. Восемь
    # метров фоновой ячейки на слой в десять сантиметров дали аспект восемьдесят
    # — и с ним скошенность 14.4 при пороге 4, неортогональность 84° и НОЛЬ
    # добавленных слоёв. Та же геометрия при аспекте десять дала скошенность
    # 2.7 и слои на 96% граней.
    for _p, m in found:
        if m["template"] != "openfoam-2d":
            continue
        c = openfoam.context(m, geom)
        aspect = c["base_size_m"] / max(c["span_m"], 1e-9)
        if aspect > 20:
            check("аспект фоновой ячейки плоского случая: " + m["case_id"],
                  False, "%.0f при пороге 20 (ячейка %.2f м, слой %.2f м)"
                  % (aspect, c["base_size_m"], c["span_m"]))
    check("у плоских случаев фоновая ячейка не вытянута сильнее чем в двадцать раз",
          all(openfoam.context(m, geom)["base_size_m"]
...
try:
    ...
    try:
        for path, m in sorted(triple, key=lambda x: x[1]["mesh"]["level"]):
            ...
            os.makedirs(d)
            # Ряд обязан ДОХОДИТЬ до окна усреднения манифеста. Прежде он
            # кончался на 3000-м шаге при окне с 5200-го, и батарея молча
            # пользовалась схлопыванием окна до двух точек — тем самым
            # откатом, который однажды соврал на эталонной сетке (§13.25).
            ts = np.linspace(0.0, float(m["numerics"]["end_time"]), 1201)
            with open(os.path.join(d, "force.dat"), "w", encoding="utf-8") as f:
