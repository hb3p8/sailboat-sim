...
//
// Что открыто. Клетка 135°/4.5 ХАОТИЧНА: от тысячной доли узла в начальном ходе
// скачок ходит через порог в обе стороны (замер на HEAD `d0929fd`: 3.4, 6.6,
// 2.9, 5.2, 4.5 % при начальных ходах 3.000…3.100). Остальные двадцать четыре
// при том же возмущении стоят побитово. Поэтому карта и меряется теперь с ДВУХ
// начальных ходов с худшим из двух — и показывает то, что было и раньше, но
// пряталось за одним прогоном.
//
