...
//
// БАТАРЕЯ ЗЕЛЁНАЯ С 2026-09-12 и с тех пор входит в `make test`.
//
// Долго она валилась нарочно: последняя проверка — карта курс×шкот, и клетка
// 120°/7.5 разносилась (Γ = 797 при фоновом 21.6, скачок тяги 17.5 %). Причина
// оказалась не в пелене, а в знаке пуза: он переключался по `Math.sign(alpha)`,
// и у сваленной по потоку полоски, где угол атаки равен нулю ТОЖДЕСТВЕННО,
// пузо прыгало с +m на −m за шаг. Чинится непрерывным знаком (`camberSign` в
// membrane.js, шаг 1 в docs/gennaker-sota-plan.md); двадцать клеток из
// двадцати пяти при этом не изменились ни в одном разряде.
//
// Полоса шкота 5.4…6.6 м при TWA 140° от этого НЕ вылечилась и лечиться не
// должна была: там своя причина — явная связь со вторым рядом пелены
// (docs/wake.md, §5.12), и её чинит шаг 2 плана. Здесь она не видна потому,
// что при шаге 1/30 решение ещё стоит; разваливается оно с 1/40 и мельче.
