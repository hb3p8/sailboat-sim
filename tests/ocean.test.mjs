...
//
// Порог был 5 см при N = 256 (выходило 2.8), стал 7 при N = 128 (выходит 5.6), и
// это не послабление ради зелёной проверки, а записанная цена решения. Сторона
// решётки уполовинена по замеру С ТЕЛЕФОНА: все вычислительные проходы вместе
// стоили там 21.9 мс на кадр — больше, чем вся отрисовка (docs/perf.md, §3).
// Вчетверо меньше элементов на каскад — вдвое длиннее самая короткая волна.
//
// Проверка при этом не выродилась: она держит границу, за которой мелкий каскад
// перестаёт быть мелким. Семь сантиметров — это ещё рябь; двадцать были бы уже
// зыбью, и тогда каскад незачем.
//
// Недостачу самой мелкой ряби добирает шейдерная рябь поверх волны — она и
// раньше рисовалась, и на неё это разрешение не влияет вовсе.
const finest = 2 * OCEAN_TILES[OCEAN_TILES.length - 1] / OCEAN_N;
check('мелкий каскад достаёт до ряби в единицы сантиметров',
  finest < 0.07, (100 * finest).toFixed(1) + ' см');
