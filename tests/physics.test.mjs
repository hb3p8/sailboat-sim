...
for (const sheet of [20, 24, 28, 32, 36, 40]) {
...
// Обмер парусов по чертежу забрал у якоря треть узла: 8.8 до обмера тем же
// протоколом, 8.4 после. Парусности стало меньше (22.85 м² против принятых
// прежде 25), и это не подгонка разъехалась, а исходные данные оказались другими:
// сумма снятых площадей сходится с парусностью из штампа самого чертежа.
// Кривая RESIDUARY при этом не трогалась; если её когда-нибудь придётся
// править, пусть будет видно, мимо какой оценки правка проехала.
check('галфвинд в 11.7 узлах ветра у рига около восьми узлов',
  anchor.t.speedKn > 7.8 && anchor.t.speedKn < 9.2,
