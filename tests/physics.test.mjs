...
// Потолок крена берётся ИЗ КРИВОЙ ВОССТАНАВЛИВАЮЩИХ ПЛЕЧ, а не назначается
// круглым числом.
//
// Прежде здесь стояло 35°, и это была плотность без замера за спиной. Под ней
// лежала ошибка: момент крена от киля и руля собирался с обратным знаком
// (sim/physics.js, `foilHeel`), кренящее плечо выходило на треть короче
// настоящего — 3.41 м вместо 4.93, — и всё, что мерялось креном, мерялось
// заниженным. С исправленным знаком эта же строка даёт 36.2°, и число сходится
// руками: остойчивость 590·9.81·GZ(36°) против боковой силы из проверенных по
// ORC коэффициентов даёт тот же угол.
//
// Строки расписания идут БЕЗ ЭКИПАЖА (`hike` в них не задан), и сшкочены под
// лучший VMG. Ненагруженная лодка в шестьсот килограммов с двумя десятками
// квадратов, добранными на полную в одиннадцать с лишним узлов, кренится сильно
// — и это не отказ модели, а её ответ.
//
// Отказ начинается там, где крен перестаёт покупать остойчивость: за вершиной
// кривой GZ прибавка крена УМЕНЬШАЕТ восстанавливающий момент, и лодка идёт
// дальше сама. Это и есть «запредельный», и точка у него не круглая, а своя у
// каждого пакета: у SV20 вершина на 50°. Проверка читает её из того же пакета,
// из которого считается остойчивость, и потому переживает пересчёт обводов.
const GZ_PEAK = PACK.righting.gz.reduce((a, r) => r.gz_m > a.gz_m ? r : a).heel_deg;
check('крен в бейдевинд не доходит до вершины кривой остойчивости',
  Math.abs(beat.heelDeg) < GZ_PEAK,
  beat.heelDeg.toFixed(1) + '° при вершине на ' + GZ_PEAK + '°');
...
console.log('\nБесконечная вода: акватория выключаема\n');
{
  ...
  {
    const v = new Terrain(null).current(0, 0, 0.55, { x: 1, y: 1 });
