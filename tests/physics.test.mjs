...
// Якорь уезжал дважды, и оба раза не от подгонки, а от исправленной ошибки.
// Обмер парусов по чертежу забрал треть узла: 8.8 → 8.4 (парусности оказалось
// 22.85 м² вместо принятых 25). Мембрана забрала ещё семь десятых: 8.4 → 7.7.
// Второе — плата за то, что индуктивное сопротивление стало считаться по
// пелене, а не по полному местному скосу. В прежнем счёте подпор от стакселя
// на грот выходил отрицательным сопротивлением, то есть тягой из ниоткуда;
// теперь качество рига (5.3 на галфвинде, 7 в лавировку) легло в коридор
// продувок, а лодка стала на десятую часть медленнее.
//
// Кривая RESIDUARY при этом опять не трогалась. Она подогнана под прежний,
// слишком мощный риг, и переподбирать её надо отдельным заходом, а не заодно с
// переделкой парусов — иначе не разобрать, что от чего. Нижняя граница
// расширена до 7.3, число записано здесь, чтобы правка не проехала молча.
