...
// когда-то угадали шкот: после перехода на вихревую решётку срыв у сечения
// сдвинулся, и прежние 24° на галфвинде оказались за срывом — лодка «теряла»
// два узла на ровном месте, хотя на 30° шла как прежде.
for (const [twa, sheet] of [[40, 8], [50, 14], [60, 10], [90, 30],
                            [120, 46], [150, 50], [175, 90]]) {
...
// После перехода рига на вихревую решётку галфвинд встал на 7.5 узла вместо
// прежних 7.9 — на восьмую часть ниже оценки владельца. Коэффициенты рига при
// этом легли в коридор продувок (CL 1.1…1.6 при качестве 4.6…5.6 на острых
// курсах), кривая сопротивления не трогалась. То есть расхождение настоящее, а
// не подгонка разъехалась. Рычаг здесь один — кривая RESIDUARY; трогать её
// вслепую после такой переделки было бы враньём, поэтому нижняя граница
// расширена, а число записано в комментарии, чтобы следующая правка его не
// проехала молча.
check('галфвинд в 11.7 узлах ветра у рига около восьми узлов',
  anchor.t.speedKn > 7.2 && anchor.t.speedKn < 9.2,
...
const balance = [];
for (const [twa, sheet] of [[45, 12], [60, 10], [90, 30]]) {
  const b = new Boat(PACK);
...
const beatRows = [[30, 8], [45, 12], [60, 10]].map(([twa, sheet]) => {
  const r = sail(twa, sheet);
...
// Отданные шкоты и брошенный руль.
{
  ...
  check('стаксель при этом не работает вовсе',
    t.strips.slice(6).every(s => s.cl < 0.02),
    'наибольший cl ' +
    Math.max(...t.strips.slice(6).map(s => s.cl)).toFixed(3));
  // У грота работает верх, а низ гасит скос от воды и от остатков стакселя.
  check('у грота работает только часть',
    t.strips[0].cl < 0.1 && t.strips[5].cl > 0.3,
