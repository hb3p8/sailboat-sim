...
//
// ЧЕГО ЗДЕСЬ НЕТ И НЕ БУДЕТ, пока не появятся наблюдения: проверок на конкретную
// скорость. Калибровочных точек у глиссирования нет ни одной — та единственная,
// что есть, лежит в вытесняющем режиме (галфвинд около восьми узлов в 11.7 узлах
// ветра у рига, батарея физики). Поэтому здесь проверяется только ФОРМА ответа,
// то есть то, что можно утверждать без данных:
//
...
//
// Числовую полку («в 12 узлах на бакштаге держит 12–14») сюда добавит первое же
// наблюдение владельца, и вот тогда модель станет проверенной, а не только
// правдоподобной.
