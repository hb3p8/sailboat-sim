...
//
// А с тех пор как силы паруса считает свободная пелена, к этому добавилось
// второе, и оно уже не про формат. У модели появилась ПАМЯТЬ на 1.3 секунды:
// четырнадцать нитей по сорок узлов, у каждого место, ядро и циркуляция кольца.
// В записи её нет и в нынешнем виде быть не может — три с половиной тысячи
// чисел на кадр против полусотни, — так что воспроизведение стартует с пустой
// пеленой. Прогрев не спасает: разница набирается не при заполнении, а от того,
// что дальше пелена растёт вдоль чуть иной траектории. Замерено: скорость
// 2.6e-3 уз, крен 0.085°, курс 0.063° за сорок пять секунд — вместо прежних
// 2e-3 / 0.02 / 0.02.
//
// Допуск поэтому раздвинут до этих величин с небольшим запасом, и это честнее,
// чем делать вид, что ничего не изменилось. Настоящее решение — класть пелену в
// запись опорными кадрами; открытый пункт в docs/wake.md.
check('прогон воспроизводится по записи',
  worst.speed < 4e-3 && worst.heel < 1.2e-1 && worst.psi < 1e-1,
  'худший момент на ' + worst.at.toFixed(1) + ' с, крен ' +
  worst.heel.toFixed(3) + '°');
