...
function best(twaDeg, opts) {
  let top = null;
  // Шаг перебора огрублён после появления вихревой решётки: шаг физики стал
  // дороже, и прежняя сетка 15×6 гоняла батарею больше получаса. Оптимум от
  // этого сдвигается на градус-другой, а поляра остаётся той же.
  let fastest = 0;
  for (let sh = 6; sh <= 32; sh += 4) {
    for (let tw = 0; tw <= 30; tw += 10) {
...
// Сетка перебора твиста огрублена до десяти градусов, поэтому «не нужен»
// здесь означает «не больше одного шага сетки».
check('в слабый ветер твист не нужен', byWind[0].top.twist <= 10,
  byWind[0].top.twist + '°');
