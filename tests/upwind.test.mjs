...
// Спад за горбом меряется НЕ ДОЛЕЙ ОТ ГОРБА, и прежняя доля — 0.8 — была
// круглым числом без замера. Под ней к тому же лежала ошибка: момент крена от
// киля и руля собирался с обратным знаком, кренящее плечо выходило на треть
// короче настоящего, и лодка несла в свежий ветер больше паруса, чем может.
// С исправленным знаком спад глубже — 2.80 против 3.52 на горбе, — и это
// правильно: в четырнадцать метров в секунду, то есть в двадцать семь узлов,
// шестиметровая лодка перегружена и лавируется плохо.
//
// Проверяется теперь два утверждения, и ни в одном нет подбираемого числа.
// Первое: за горбом спад МОНОТОННЫЙ — модель, у которой VMG за горбом скачет,
// сломана, сколько бы она ни выдавала. Второе: даже в самый сильный ветер
// сетки лавироваться не хуже, чем в самый слабый, — обвал ниже этого означает,
// что лодка перестала идти, а не что её прижало.
const vmgLight = vmgs[0];
check('за горбом VMG спадает монотонно',
  vmgs.slice(iPeak).every((v, i) => i === 0 || v <= vmgs[iPeak + i - 1] + 0.01),
  vmgs.slice(iPeak).map(v => v.toFixed(2)).join(' → '));
check('за горбом VMG не обваливается ниже слабого ветра',
  vmgs.slice(iPeak).every(v => v > vmgLight),
  'наименьший ' + Math.min(...vmgs.slice(iPeak)).toFixed(2) + ' против ' +
  vmgLight.toFixed(2) + ' в ' + byWind[0].wind + ' м/с; горб ' +
  peak.toFixed(2));
