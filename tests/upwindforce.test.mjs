...
// Сетка идёт до самого добранного шкота: максимум подъёмной силы лежит на
// тугом триме, и на прежней сетке (от 8°) он упирался в её край.
const SHEETS = pick([0, 2, 4, 6, 9, 12, 16, 20, 25, 30, 35], [0, 3, 6, 9, 14, 20, 28]);
const TWISTS = pick([0, 5, 10, 15, 20], [0, 8, 16]);
...
// И отдельно — то, что первый прогон вскрыл: на галфвинде подъёмная сила
// проваливается. Полоса здесь шире, потому что там и эталон слабее (у ORC
// собственный грот со стакселем на таком угле уже частично сорван), но полтора
// раза — это не разброс.
{
  ...
  if (wide.length) check('на галфвинде CL не проваливается больше чем на четверть',
    worstRel <= 0.25,
    'худшее ' + (100 * worstRel).toFixed(0) + ' % на AWA ' + worstAt.toFixed(0) + '°');
}
