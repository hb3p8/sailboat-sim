...
//
// Гоняется на ДВУХ геометриях. В первой пелена сходит прямо со связанного вихря
// (TA = A), как у нынешнего рига, — и это случай вырожденный: отрезков вдоль
// хорды в контуре нет, так что мимо расхождения в них тест проедет. Во второй
// точки схода отодвинуты назад, и контур замкнут полностью. Проверять нужно
// именно вторую: пока `induced` не считал отрезки вдоль хорды, на вырожденной
// геометрии расхождение было 5e-18, а на сдвинутой — 4.4e-1, то есть половина
// величины. Никакая лодка этого бы не показала, потому что у неё TA = A.
for (const shifted of [false, true]) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const tx = shifted ? x + 0.75 : x;          // точка схода: на вихре или позади
    p.a[0] = x; p.a[1] = y0; p.a[2] = z;
    p.b[0] = x; p.b[1] = y1; p.b[2] = z;
    p.ta[0] = tx; p.ta[1] = y0; p.ta[2] = z;
    p.tb[0] = tx; p.tb[1] = y1; p.tb[2] = z;
  ...
  const what = shifted ? 'точки схода отодвинуты назад' : 'пелена сходит со связанного';
  console.log('Скорость в точке против матрицы влияния (' + what + '): ' +
    worst.toExponential(1));
  check('induced совпадает с матрицей влияния, ' + what,
