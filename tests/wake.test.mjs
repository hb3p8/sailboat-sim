...
// Здесь проверяется, что пелена даёт его сама.
{
  ...
  // Точного совпадения с Вагнером тут быть НЕ ДОЛЖНО, и это не оговорка задним
  // числом. Вагнер — плоская задача: крыло бесконечного размаха, жёсткое, одно.
  // Здесь размах конечный и невелик, парусов два, снизу зеркало от воды, а сам
  // парус мягкий. Крыло конечного размаха выходит на новую подъёмную силу
  // БЫСТРЕЕ плоского: сошедшая завихренность уходит не только назад, но и вбок.
  // Поэтому 0.85 на второй полухорде против плоских 0.67 — ожидаемо, а строгая
  // сверка требует отдельного стенда с жёстким прямоугольным крылом.
  //
  // Проверяется здесь то, что от постановки не зависит: запаздывание ЕСТЬ (не
  // единица, как у прямых лучей), оно в разумных пределах, и к дюжине полухорд
  // циркуляция выходит на установившуюся.
  check('пелена держит циркуляцию, пока разгонный вихрь рядом',
    free(2) > 0.5 && free(2) < 0.95, free(2).toFixed(2));
  check('и отпускает её к установившейся', free(12) > 0.92 && free(12) < 1.05,
    free(6).toFixed(2) + ' на шести полухордах, ' + free(12).toFixed(2) + ' на двенадцати');
}
