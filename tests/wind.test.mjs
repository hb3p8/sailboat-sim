...
// шкаторины должен сойти разгонный вихрь. Половина прироста приходит мгновенно,
// вторая набегает за несколько хорд пути.
//
// Лодка на время замораживается: мерить надо парус, а не разгон. Иначе в ответ
// подмешается инерция корпуса, а она на порядок медленнее.
{
  const p = new Boat(PACK);
  ...
  hold(600);
  const alpha = () => p.telemetry.strips[2].alphaDeg;
  const before = cl(), beforeA = alpha();
  p.o.sheet = 26 * D;
  hold(1);
  const jump = cl(), jumpA = alpha();
  const trace = [];
  for (let i = 0; i < 200; i++) { hold(1); trace.push(cl()); }
  hold(900);
  const after = cl(), afterA = alpha();
  ...
  console.log('  cl третьей полоски грота ' + before.toFixed(3) + ' -> сразу ' +
    jump.toFixed(3) + ' -> установившаяся ' + after.toFixed(3));
  console.log('  угол атаки ' + beforeA.toFixed(1) + '° -> сразу ' +
    jumpA.toFixed(1) + '° -> установившийся ' + afterA.toFixed(1) + '°');
  console.log('  постоянная времени ' + tau.toFixed(2) + ' с, это ' +
    (tau * ve / chord).toFixed(1) + ' хорды пути (хорда ' +
    chord.toFixed(2) + ' м, поток ' + ve.toFixed(1) + ' м/с)\n');
  // «Половина сразу» — свойство циркуляции, то есть угла атаки, а не cl:
  // кривая сечения нелинейна, и у самого горба половина угла даёт заметно
  // больше половины подъёмной силы.
  const half = (jumpA - beforeA) / (afterA - beforeA);
  check('половина прироста угла атаки приходит сразу',
    half > 0.4 && half < 0.6, (100 * half).toFixed(0) + '%');
  check('вторая половина набегает за одну-четыре хорды пути',
    tau * ve / chord > 1 && tau * ve / chord < 4,
    (tau * ve / chord).toFixed(1) + ' хорды');
