...
// берутся не из ветра, а из ошибки.
{
  ...
  check('при однородном ветре у всех полосок один угол атаки',
    spread(a.map(s => s.alphaDeg)) < 1e-9,
    'разброс ' + spread(a.map(s => s.alphaDeg)).toExponential(1) + '°');
...
// С профилем ветра верх паруса обязан видеть больше ветра и свободнее угол.
{
  ...
  check('вверху кажущийся ветер свободнее', a[5].awaDeg > a[0].awaDeg + 2,
  ...
  // уводится в отрицательный угол атаки и перестаёт работать вовсе.
  check('большой твист уводит верх паруса в заполаскивание',
    open[5].alphaDeg < 0 && open[0].alphaDeg > 4,
    'низ ' + open[0].alphaDeg.toFixed(1) + '°, верх ' +
    open[5].alphaDeg.toFixed(1) + '°');
}
