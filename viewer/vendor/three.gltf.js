  ...
  loadNode(nodeIndex) {
    ...
    return Promise.all([
      ...
      if (node.userData.pivot !== void 0 && children.length > 0) {
        ...
        node.pivot = new Vector33().fromArray(pivot);
...
function computeBounds(geometry, primitiveDef, parser) {
  ...
  if (attributes.POSITION !== void 0) {
    ...
    if (min !== void 0 && max !== void 0) {
      box.set(
        new Vector33(min[0], min[1], min[2]),
        new Vector33(max[0], max[1], max[2])
      );
      if (accessor.normalized) {
...
// package/examples/jsm/loaders/DRACOLoader.js
import {
...
var DRACOLoader = class extends Loader2 {
  ...
  setDecoderPath(path) {
    ...
    if (typeof path === "object") {
      ...
      decoderPaths.js = LoaderUtils2.resolveURL("draco_wasm_wrapper.js", path);
      decoderPaths.wasm = LoaderUtils2.resolveURL("draco_decoder.wasm", path);
      decoderPaths.dep_js = LoaderUtils2.resolveURL("draco_decoder.js", path);
    }
    return this;
  }
  /**
  ...
  parse(buffer, onLoad, onError = () => {
    ...
    this.decodeDracoFile(buffer, onLoad, null, null, SRGBColorSpace2, onError).catch(onError);
  }
  //
  ...
  _createGeometry(geometryData) {
    ...
    for (let i = 0; i < geometryData.attributes.length; i++) {
      ...
      let attribute;
      if (itemSize === stride) {
        attribute = new BufferAttribute3(array, itemSize);
      } else {
        const buffer = new InterleavedBuffer3(array, stride);
        attribute = new InterleavedBufferAttribute3(buffer, itemSize, 0);
      }
      if (name === "color") {
  ...
  _assignVertexColorSpace(attribute, inputColorSpace) {
    ...
    const _color = new Color2();
    for (let i = 0, il = attribute.count; i < il; i++) {
      _color.fromBufferAttribute(attribute, i);
      ColorManagement2.colorSpaceToWorking(_color, SRGBColorSpace2);
